...
//---------------------------------------------------------------------------
void __fastcall TIDCGen::MakeComment(int pos, AnsiString text)
{
...
int __fastcall TIDCGen::OutputRTTIHeader(BYTE kind, int pos)
{
    ...
    BYTE len = *(Code + pos + 5);
    itemName = AnsiString((LPTSTR)(Code + pos + 6), len);
    DWORD adr = Pos2Adr(pos);
    CurrentBytes += fprintf(idcF, "MakeUnkn(0x%lX, 1);\n", adr);
...
//---------------------------------------------------------------------------
int __fastcall TIDCGen::OutputVMTHeader(int pos, AnsiString vmtName)
{
    int fromPos = pos;
    DWORD adr = Pos2Adr(pos);
...
//---------------------------------------------------------------------------
__fastcall TSaveIDCDialog::TSaveIDCDialog(TComponent* AOwner, LPTSTR TemplateName) : TOpenDialog(AOwner)
{
	Options >> ofEnableSizing;
	Template = (LPWSTR)TemplateName;
