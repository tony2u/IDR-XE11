...
//---------------------------------------------------------------------------
void __fastcall InfoVmtInfo::AddInterface(AnsiString Value)
{
    if (!interfaces) interfaces = new TStringList;
    interfaces->Add(Value);
}
//---------------------------------------------------------------------------
PFIELDINFO __fastcall InfoVmtInfo::AddField(DWORD ProcAdr, int ProcOfs, BYTE Scope, int Offset, int Case, AnsiString Name, AnsiString Type)
{
	PFIELDINFO	fInfo = 0;
...
//---------------------------------------------------------------------------
bool __fastcall InfoVmtInfo::AddMethod(bool Abstract, TCHAR Kind, int Id, DWORD Address, AnsiString Name)
{
	PMethodRec	recM;
...
//---------------------------------------------------------------------------
PARGINFO __fastcall InfoProcInfo::AddArg(BYTE Tag, int Ofs, int Size, AnsiString Name, AnsiString TypeDef)
{
    PARGINFO 	argInfo;
...
//---------------------------------------------------------------------------
AnsiString __fastcall InfoProcInfo::AddArgsFromDeclaration(LPTSTR Decl, int from, int callKind)
{
    bool    fColon;
    TCHAR    *p, *pp, *cp, c, sc;
    int     ss, num;
            ...
            for (int arg = from;;arg++)
            {
                ...
                *pp = 0;
                ...
                if (_Name[0] == '?' && _Name[1] == 0)
                    ...
                    argInfo.Name = AnsiString(_Name).Trim();

                if (_Type[0] == '?' && _Type[1] == 0)
                    argInfo.TypeDef = "";
                else
                    argInfo.TypeDef = TrimTypeName(AnsiString(_Type));
...
//---------------------------------------------------------------------------
PLOCALINFO __fastcall InfoProcInfo::AddLocal(int Ofs, int Size, AnsiString Name, AnsiString TypeDef)
{
    PLOCALINFO  locInfo;
...
//---------------------------------------------------------------------------
PLOCALINFO __fastcall InfoProcInfo::GetLocal(AnsiString Name)
{
    if (locals)
    {
        for (int n = 0; n < locals->Count; n++)
...
//---------------------------------------------------------------------------
void __fastcall InfoProcInfo::SetLocalType(int Ofs, AnsiString TypeDef)
{
    PLOCALINFO locInfo = GetLocal(Ofs);
    if (locInfo)
    {
        AnsiString  fname = locInfo->Name;
        int pos = fname.Pos(".");
        if (pos)
                    ...
                    if (str.Pos(TypeDef + "=") == 1)
                    {
                        while (1)
                        {
                            ...
                            if (str.Pos("//"))
                            {
                                ...
                                AnsiString name = StrGetRecordFieldName(str);
                                AnsiString type = StrGetRecordFieldType(str);
                                if (ofs >= 0)
                ...
                if (KnowledgeBase.GetTypeInfo(idx, INFO_FIELDS, &tInfo))
                {
                    if (tInfo.FieldsNum)
                    {
                        ...
                        for (int k = 0; k < tInfo.FieldsNum; k++)
                        {
                            ...
                            //Name
                            int len = *((WORD*)p); p += 2;
                            AnsiString name = AnsiString((LPTSTR)p, len); p += len + 1;
                            //Type
                            len = *((WORD*)p); p += 2;
                            AnsiString type = TrimTypeName(AnsiString((LPTSTR)p, len)); p += len + 1;
...
//---------------------------------------------------------------------------
AnsiString __fastcall InfoRec::GetName()
{
    return name;
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void __fastcall InfoRec::SetName(AnsiString AValue)
{
    CrtSection->Enter();
    name = AValue;
...
//---------------------------------------------------------------------------
void __fastcall InfoRec::ConcatName(AnsiString AValue)
{
    name += AValue;
}
//---------------------------------------------------------------------------
bool __fastcall InfoRec::SameName(AnsiString AValue)
{
    return SameText(name, AValue);
}
//---------------------------------------------------------------------------
void __fastcall InfoRec::AddXref(TCHAR Type, DWORD Adr, int Offset)
{
    PXrefRec 	recX;
...
//---------------------------------------------------------------------------
void __fastcall InfoRec::ScanUpItemAndAddRef(int fromPos, DWORD itemAdr, TCHAR refType, DWORD refAdr)
{
    while (fromPos >= 0)
    {
        fromPos--;
...
//---------------------------------------------------------------------------
void __fastcall InfoRec::Load(TStream* ins, LPTSTR buf)
{
    ...
    ins->Read(buf, len);
    name = AnsiString(buf, len);
    //type
    ins->Read(&len, sizeof(len));
    ins->Read(buf, len);
    type = AnsiString(buf, len);
    //picode
    ...
    if (picode)
    {
        ...
        ins->Read(&len, sizeof(len));
        ins->Read(buf, len);
        picode->Name = AnsiString(buf, len);
    }
    //xrefs
    ...
    if (kind == ikResString)
    {
        ...
        ins->Read(&len, sizeof(len));
        ins->Read(buf, len);
        rsInfo->value = AnsiString(buf, len);
    }
    else if (kind == ikVMT)
    {
    	//interfaces
        ...
        for (m = 0; m < num; m++)
        {
            ins->Read(&len, sizeof(len));
            ins->Read(buf, len);
            vmtInfo->interfaces->Add(AnsiString(buf, len));
        }
    	//fields
        ins->Read(&num, sizeof(num));
        ...
        for (m = 0; m < num; m++)
        {
            ...
            ins->Read(&len, sizeof(len));
            ins->Read(buf, len);
            fInfo->Name = AnsiString(buf, len);
            //Type
            ins->Read(&len, sizeof(len));
            ins->Read(buf, len);
            fInfo->Type = AnsiString(buf, len);
            //xrefs
    ...
    else if (kind >= ikRefine && kind <= ikFunc)
    {
        ...
        for (m = 0; m < num; m++)
        {
            ...
            ins->Read(&len, sizeof(len));
            ins->Read(buf, len);
            argInfo->Name = AnsiString(buf, len);
            //TypeDef
            ins->Read(&len, sizeof(len));
            ins->Read(buf, len);
            argInfo->TypeDef = TrimTypeName(AnsiString(buf, len));
    ...
    //locals
        ...
        for (m = 0; m < num; m++)
        {
            ...
            ins->Read(&len, sizeof(len));
            ins->Read(buf, len);
            locInfo->Name = AnsiString(buf, len);
            //TypeDef
            ins->Read(&len, sizeof(len));
            ins->Read(buf, len);
            locInfo->TypeDef = TrimTypeName(AnsiString(buf, len));
...
//---------------------------------------------------------------------------
/*
void __fastcall InfoRec::Skip(TStream* ins, LPTSTR buf, BYTE asKind)
{
    DWORD       dummy;
...
*/
//---------------------------------------------------------------------------
AnsiString __fastcall InfoRec::MakePrototype(int adr, bool showKind, bool showTail, bool multiline, bool fullName, bool allArgs)
{
    BYTE        callKind;
...
//---------------------------------------------------------------------------
AnsiString __fastcall InfoRec::MakeDelphiPrototype(int Adr, PMethodRec recM)
{
    bool        abstract = false;
    BYTE        callKind;
    ...
    return AnsiString(StringBuf, len);
}
//---------------------------------------------------------------------------
AnsiString __fastcall InfoRec::MakeMultilinePrototype(int Adr, int* ArgsBytes, AnsiString MethodType)
{
    BYTE        callKind;
    ...
    for (n = firstArg; n < argsNum; n++)
    {
        ...
        if (argInfo->TypeDef != "")
            ...
            result += "?";
        //size
        if (argInfo->Size > 4) result += ":" + AnsiString(argInfo->Size);
...
//---------------------------------------------------------------------------
AnsiString __fastcall InfoRec::MakeMapName(int Adr)
{
    AnsiString result;
    if (name != "")
        result = name;
...
bool __fastcall InfoRec::MakeArgsManually()
{
    ...
    if (SameText(name, "@ReadLString"))
    {
        kind = ikProc;
        procInfo->AddArg(0x22, 0, 4, "T", "TTextRec");
        procInfo->AddArg(0x22, 1, 4, "S", "String");
        return true;
    }
    //@ReadWString
    ...
    {
        kind = ikProc;
        procInfo->AddArg(0x21, 0, 4, "s", "String");
...
procedure _Str2Ext(val:Extended; width:Longint; precision:Longint; var s:AnsiString); [ESP+4],EAX,EDX,ECX
procedure _Str0Ext(val:Extended; var s:AnsiString); [ESP+4],EAX
procedure _Str1Ext(val:Extended; width:Longint; var s:AnsiString); [ESP+4],EAX,EDX
