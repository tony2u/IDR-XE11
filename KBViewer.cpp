...
void __fastcall TFKBViewer_11011981::ShowCode(DWORD adr, int idx)
{
    ...
    if (KnowledgeBase.GetProcInfo(CurrIdx, INFO_DUMP, pInfo))
    {
        ...
        if (pInfo->FixupNum)
        {
            TCHAR *p = pInfo->Dump + 2*pInfo->DumpSz;
            ...
            for (n = 0; n < pInfo->FixupNum; n++)
            {
                ...
                WORD len = *((WORD*)p); p += 2;
                AnsiString fixupName = AnsiString(p, len); p += len + 1;
...
void __fastcall TFKBViewer_11011981::btnOkClick(TObject *Sender)
{
    ...
    MProcInfo   *pInfo = &aInfo;
    PInfoRec    recN;
    AnsiString      kbName, idrName, kbLine, idrLine;
            ...
            for (m = 0; m < lbKB->SelCount; m++)
            {
                ...
                if (kbLine != "" && idrLine != "")
                {
                    ...
                    if (pos1 && pos2)
                    {
                        ...
                        adr = 0;
                        if (TryStrToInt(AnsiString("$") + idrName, val)) adr = val;
                        ap = Adr2Pos(adr);
                        if (kbName != "" && ap >= 0)
                        {
                            recN = GetInfoRec(adr);
                            recN->procInfo->DeleteArgs();
							LPTSTR text = TransformPChar(CP_ACP, cbUnits->Text.c_str(), -1);
							WORD *uses = KnowledgeBase.GetModuleUses(KnowledgeBase.GetModuleID(text));
...
void __fastcall TFKBViewer_11011981::cbUnitsChange(TObject *Sender)
{
	...
	Position = -1;
	LPTSTR text = TransformPChar(CP_ACP, cbUnits->Text.c_str(), -1);
	_moduleID = KnowledgeBase.GetModuleID(text);
	delete[] text;
    if (_moduleID != 0xFFFF)
    {
...
void __fastcall TFKBViewer_11011981::FormShow(TObject *Sender)
{
    if (!UnitsNum)
    {
      for (int n = 0; n < KnowledgeBase.ModuleCount; n++)
      {
          ...
          cbUnits->Items->Add(AnsiString((LPTSTR)(p + 4)));
          UnitsNum++;
      }
    }
}
