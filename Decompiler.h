...
typedef struct
{
    TCHAR        state;          //'U' not defined; 'B' branch; 'J' jump; '@' label; 'R' return; 'S' switch
    int         bcnt;           //branches to... count
    DWORD       address;
    AnsiString      dExpr;          //condition of direct expression
    AnsiString      iExpr;          //condition of inverse expression
    AnsiString      result;
} TBJLInfo;
...
typedef struct
{
    ...
    DWORD   Flags;
    AnsiString  Value;
    AnsiString  Value1;     //For various purposes
    AnsiString  Type;
    AnsiString  Name;
} ITEM, *PITEM;
...
public:
    ...
    IDXINFO CntInfo;
public:
    __fastcall TForInfo(bool ANoVar, bool ADown, int AStopAdr, AnsiString AFrom, AnsiString ATo, BYTE AVarType, int AVarIdx, BYTE ACntType, int ACntIdx);
};
...
public:
    ...
    __fastcall TNamer();
    __fastcall ~TNamer();
    AnsiString __fastcall MakeName(AnsiString shablon);
};
...
{
public:
    AnsiString      ProcName;       //Name of decompiled procedure
    ...
    AnsiString __fastcall GetLvarName(int Ofs, AnsiString Type);
    void __fastcall AssignItem(PITEM DstItem, PITEM SrcItem);
    void __fastcall AddToBody(AnsiString src);
    void __fastcall AddToBody(TStringList* src);
    bool __fastcall IsExitAtBodyEnd();

    void __fastcall OutputSourceCodeLine(AnsiString line);
    ...
    bool __fastcall BJLCheckPattern1(LPTSTR t, int from);
    bool __fastcall BJLCheckPattern2(LPTSTR t, int from);
    ...
    TCHAR __fastcall ExprGetOperation(AnsiString s);
    void __fastcall ExprMerge(AnsiString& dst, AnsiString src, TCHAR op);//dst = dst op src, op = '|' or '&'
    AnsiString __fastcall PrintBJL();
...
public:
    ...
    int __fastcall GetArrayFieldOffset(AnsiString ATypeName, int AFromOfs, int AScale, AnsiString& _name, AnsiString& _type);
    int __fastcall GetCmpInfo(DWORD fromAdr);
    AnsiString __fastcall GetCycleFrom();
    void __fastcall GetCycleIdx(PIDXINFO IdxInfo, DISINFO* ADisInfo);
    AnsiString __fastcall GetCycleTo();
    ...
    AnsiString __fastcall GetStringArgument(PITEM item);
    ...
    AnsiString __fastcall GetRegType(int Idx);
    AnsiString __fastcall GetSysCallAlias(AnsiString AName);
    ...
    bool __fastcall SimulateSysCall(AnsiString name, DWORD procAdr, int instrLen);
