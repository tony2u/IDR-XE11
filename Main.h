...
#define     ikWChar         0x09
#define     ikLString       0x0A   //AnsiString, AnsiString
...
typedef struct
{
	...
	TCHAR       source; //0 - not defined; 'L' - local var; 'A' - argument; 'M' - memory; 'I' - immediate
    DWORD       value;
	AnsiString  type;
} RINFO, *PRINFO;
//---------------------------------------------------------------------------
...
__published:	// IDE-managed Components
    ...
    void __fastcall FindText(AnsiString Str);
...
private:	// User declarations
    ...
    bool __fastcall IsExe(AnsiString FileName);
    bool __fastcall IsIdp(AnsiString FileName);
    void __fastcall LoadFile(AnsiString FileName, int version);
    void __fastcall LoadDelphiFile(int version);
    void __fastcall LoadDelphiFile1(AnsiString FileName, int version, bool loadExp, bool loadImp);
    void __fastcall ReadNode(TStream* stream, TTreeNode* node, TCHAR* buf);
    void __fastcall OpenProject(AnsiString FileName);
    ...
    int __fastcall GetUnits2(AnsiString dprName);   //For Delphi2 (other structure!)
    int __fastcall GetUnits(AnsiString dprName);
    int __fastcall GetBCBUnits(AnsiString dprName);
    ...
    void __fastcall SaveProject(AnsiString FileName);
    void __fastcall CloseProject();
    void __fastcall CleanProject();
    void __fastcall IniFileRead();
    void __fastcall IniFileWrite();
    void __fastcall AddExe2MRF(AnsiString FileName);
    void __fastcall AddIdp2MRF(AnsiString FileName);
    int __fastcall GetSegmentNo(DWORD Adr);
    int __fastcall CodeGetTargetAdr(AnsiString line, DWORD* trgAdr);
    void __fastcall OutputLine(FILE* outF, BYTE flags, DWORD adr, AnsiString content);
	void __fastcall OutputCode(FILE* outF, DWORD fromAdr, AnsiString prototype, bool onlyComments);
    //Drag&Drop
    TDragDropHelper dragdropHelper;
    void __fastcall wm_dropFiles(TWMDropFiles& msg);
    void __fastcall DoOpenProjectFile(AnsiString FileName);
    ...
    //Tree view booster
    typedef std::map<const AnsiString, TTreeNode*> TTreeNodeNameMap;
...
public:		// User declarations
    ...
    PFIELDINFO __fastcall GetField(AnsiString TypeName, int Offset, bool* vmt, DWORD* vmtAdr, AnsiString prefix);
    PFIELDINFO __fastcall AddField(DWORD ProcAdr, int ProcOfs, AnsiString TypeName, BYTE Scope, int Offset, int Case, AnsiString Name, AnsiString Type);
    int __fastcall GetMethodOfs(PInfoRec rec, DWORD procAdr);
    PMethodRec __fastcall GetMethodInfo(PInfoRec rec, AnsiString name);
    PMethodRec __fastcall GetMethodInfo(DWORD adr, TCHAR kind, int methodOfs);
    ...
    bool __fastcall IsUnitExist(AnsiString Name);
    void __fastcall SetVmtConsts(int version);
    PUnitRec __fastcall GetUnit(DWORD Adr);
    AnsiString __fastcall GetUnitName(PUnitRec recU);
    AnsiString __fastcall GetUnitName(DWORD Adr);
    void __fastcall SetUnitName(PUnitRec recU, AnsiString name);
    ...
    TTreeNode* __fastcall GetNodeByName(AnsiString AName);
    ...
    void __fastcall ScanMethodTable(DWORD adr, AnsiString className);
    ...
    AnsiString __fastcall GetCommonType(AnsiString Name1, AnsiString Name2);
    ...
    int __fastcall AddAsmLine(DWORD Adr, AnsiString text, BYTE Flags);
    ...
    void __fastcall AnalyzeProc1(DWORD fromAdr, TCHAR xrefType, DWORD xrefAdr, int xrefOfs, bool maybeEmb);
    ...
    AnsiString __fastcall AnalyzeTypes(DWORD parentAdr, int callPos, DWORD procAdr, PRINFO registers);
    AnsiString __fastcall AnalyzeArguments(DWORD fromAdr);
    ...
    TTreeNode* __fastcall AddClassTreeNode(TTreeNode* node, AnsiString name);
    //Function
    void __fastcall EditFunction(DWORD Adr);
    AnsiString __fastcall MakeComment(PPICODE Picode);
    void __fastcall InitAliases(bool find);
    void __fastcall CopyAddress(AnsiString line, int ofs, int bytes);
    ...
    void __fastcall GoToAddressMap(AnsiString cmdAdr);
    void __fastcall ExploreAdrMap(AnsiString cmdAdr);
    ...
    void __fastcall AddTreeNodeWithName(TTreeNode* node, const AnsiString& name);
    TTreeNode* __fastcall FindTreeNodeByName(const AnsiString& name);

    //show Form by its dfm object
    void __fastcall ShowDfm(TDfm* dfm);
    //bool __fastcall ImportIdp(AnsiString fileName);
};
//---------------------------------------------------------------------------
