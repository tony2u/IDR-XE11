...
void __fastcall TFEditFieldsDlg_11011981::ShowFields()
{
    ...
    if (fieldsNum)
    {
    	...
    	for (int n = 0; n < fieldsNum; n++)
        {
            ...
            AnsiString line = Val2Str5(fInfo->Offset) + " ";
            if (fInfo->Name != "")
            	line += fInfo->Name;
            else
            	line += "?";
...
void __fastcall TFEditFieldsDlg_11011981::lbFXrefsDblClick(TObject *Sender)
{
    ...
    AnsiString item = lbFXrefs->Items->Strings[lbFXrefs->ItemIndex];
...
void __fastcall TFEditFieldsDlg_11011981::bApplyClick(TObject *Sender)
{
    ...
    case FD_OP_ADD:
	case FD_OP_DELETE:
		LPTSTR text = TransformPChar(CP_ACP, edtOffset->Text.Trim().c_str(), -1);
		sscanf(text, "%lX", &offset);
		delete[] text;
        if (Op == FD_OP_ADD)
        {
