...
//---------------------------------------------------------------------------
AnsiString  IDRVersion = "01.04.2017"; 
//---------------------------------------------------------------------------
...
//ImageBase   CodeBase
//---------------------------------------------------------------------------
TCHAR    StringBuf[MAXSTRBUFFER];    //Buffer to make AnsiString

extern  LPTSTR   Reg32Tab[8];
extern  LPTSTR   Reg16Tab[8];
extern  LPTSTR   Reg8Tab[8];
extern  LPTSTR   SegRegTab[8];
extern  LPTSTR   RepPrefixTab[4];
...
MDisasm         Disasm;				//Code analyzer disassembler
...
TAnalyzeThread  *AnalyzeThread = 0; //Backend code analysis thread
...
//Common variables
AnsiString          IDPFile;
int             MaxBufLen;      //Maximum buffer length (for download)
int             DelphiVersion;
DWORD           EP;
DWORD           ImageBase;
DWORD           ImageSize;
...
DWORD           CurProcAdr;
int				CurProcSize;
AnsiString          SelectedAsmItem = "";    //Selected item in Asm Listing
AnsiString          SelectedSourceItem = ""; //Selected item in Source Code
DWORD           CurUnitAdr;
...
typedef std::map<const AnsiString, DWORD> TClassAdrMap;
TClassAdrMap classAdrMap;

void __fastcall ClearClassAdrMap();
//AnsiString __fastcall UnmangleName(LPTSTR Name);
                ...
                if (!Infos[i])
                {
                    ...
                    else
                    {
                        ...
                        if (b)
                        {
                            ...
                            {
                                if (*(e - 1) != '@')
                                    recN->SetName(AnsiString(b + 1, e - b - 1));
                                else
                                    recN->SetName(AnsiString(b + 1, e - b - 2));
...
//---------------------------------------------------------------------------
//"Sewing" VMT from POS position into code
void __fastcall TFMain_11011981::StrapVMT(int pos, int ConstId, MConstInfo* ConstInfo)
{
    ...
    for (int n = 0; n < ConstInfo->FixupNum; n++)
    {
        ...
        {
            //InitTable Link that represents the types that will be encountered in the future
            continue;
        }
        if (VMTOffset == cVmtTypeInfo)
        {
            //Type information processed, skipping
            continue;
        }
        if (VMTOffset == cVmtFieldTable)
        {
            //Skip because we will process the field information later
            continue;
        }
        if (VMTOffset == cVmtMethodTable)
        {
            //Skip because the method will be handled in other fixed items
            continue;
        }
        if (VMTOffset == cVmtDynamicTable)
        {
            //Skip because dynamic calls will be handled in other fixed items
            continue;
        }
        if (VMTOffset == cVmtClassName)
        {
            //ClassName Not processed
            continue;
        }
        if (VMTOffset == cVmtParent)
        {
            //Specify the parent class and do not handle it as it will still appear separately
            continue;
        }
        ...
        //If the address is in the code snippet and is not related to it
        if (IsValidCodeAdr(Adr) && !Infos[Adr2Pos(Adr)])
        {
            //Type name?
...
bool __fastcall TFMain_11011981::StrapCheck(int pos, MProcInfo* ProcInfo)
{
    ...
    for (int n = 0; n < ProcInfo->FixupNum; n++)
    {
        ...
        AnsiString fName = AnsiString(fixupInfo.Name);
            ...
            if (IsValidCodeAdr(Adr))
            {
                ...
                if (recN && recN->HasName())
                {
                    ...
                    else
                    {
                        ...
                        {
                            AnsiString name1 = name.SubString(1, name.Length() - 1);//Trim last symbol ('A','W') - GetWindowLongW(A)
...
pos = pos;
    ...
    AnsiString ModuleName = KnowledgeBase.GetModuleName(ProcInfo->ModuleID);
        ...
        if (!recN->MakeArgsManually())
        {
            ...
            if (p)
            {
                for (int k = 0; k < ProcInfo->ArgsNum; k++)
                {
                    ...
                    argInfo.Name = AnsiString((LPTSTR)p, wlen); p += wlen + 1;
                    wlen = *((WORD*)p); p += 2;
                    argInfo.TypeDef = TrimTypeName(AnsiString((LPTSTR)p, wlen)); p += wlen + 1;
    ...
    if (useFixups && ProcInfo->FixupNum)
    {
        ...
        for (int n = 0; n < ProcInfo->FixupNum; n++)
        {
            ...
            Len = *((WORD*)p); p += 2;
            fixupInfo.Name = p; p += Len + 1;
            fName = AnsiString(fixupInfo.Name, Len);
            //Fixupname begins with _Dn_ - skip it
            if (fName.Pos("_Dn_") == 1) continue;
            //Fixupname begins with _NF_ - skip it
            if (fName.Pos("_NF_") == 1) continue;
            ...
            //Firstly, we calculate the address, and then we attempt to determine a partition.
            if (fixupInfo.Type == 'A' || fixupInfo.Type == 'S' || fixupInfo.Type == '4' || fixupInfo.Type == '8')
            {
                //Look at the fixed position value in the dump
            ...
            {
                ShowMessage("Unknown fixup type:" + AnsiString(fixupInfo.Type));
            }

            bool isHInstance = (stricmp(fixupInfo.Name, "HInstance") == 0);
            if (!IsValidImageAdr(Adr))
            {
                //So far, only ThreadVars and TlsLast have been seen here.
            ...
            //Function external code segment address
            if (IsValidCodeAdr(Adr))
            {
                recN = GetInfoRec(Adr);
                if (!recN)
                {
                    ...
                    case KB_CONST_SECTION:
                        ...
                        if (Idx != -1)
                        {
                            ...
                            //If the name starts with _DV_, it indicates VMT
...
int __fastcall TFMain_11011981::GetDelphiVersion()
{
	...
	TControlInstSize = 0;
	//Try Dede first. (Search for TControl type)
    for (int n = 0; n < TotalSize - 14; n += 4)
    {
            ...
            Image[n + 8] == 'o' && Image[n + 9] == 'l')
        {
            ...
            if (IsValidImageAdr(vmtAdr))
            {
                ...
                if (TControlInstSize == 0x190) break;	//2006 perhaps 2007
                //Check displacement -0x34
    ...
    //Check other options through the knowledge base
    for (int v = 0; DelphiVersions[v]; v++)
    {
        ...
        if (SysKB.Open(KBFileName.c_str()))
        {
            ...
            if (moduleID != 0xFFFF)
            {
                //Search for the "System" function index in this module
                ...
                if (idx != -1)
                {
                    ...
                    if (pos != -1)
                    {
                        ...
                        {
                            //Search for function index "@Halt0" in this module
    ...
    */
    ...
    typedef struct
    {
        LPTSTR versionVcl;     //digital part of version present in vclXXX file name
        LPTSTR versionRtl;     //digital part of version present in rtlXXX file name
        LPTSTR versionString;  //full AnsiString of vcl/rtl bpl from file properties
    ...
    AnsiString rtlBplName, vclBplName, rtlBpl, vclBpl;
    ...
    if (rtlBpl != "" || vclBpl != "")
    {
        ...
        {
            rtlBplName = AnsiString(delphiBplVer[i].versionRtl);
            vclBplName = AnsiString(delphiBplVer[i].versionVcl);
            ...
            AnsiString srcPath = ExtractFilePath(SourceFile);
            AnsiString rtlFile = !rtlBplName.IsEmpty() ? srcPath+rtlBplName : rtlBplName;
            AnsiString vclFile = srcPath+vclBplName;
            ...
            AnsiString rtlBplVersion = GetModuleVersion(rtlFile);
            AnsiString vclBplVersion = GetModuleVersion(vclFile);
            //hack for D3 and 4
            if (rtlBplVersion == "") rtlBplVersion = vclBplVersion;
            if (rtlBplVersion == vclBplVersion && rtlBplVersion == AnsiString(delphiBplVer[i].versionString))
...
//---------------------------------------------------------------------------
bool __fastcall TFMain_11011981::IsUnitExist(AnsiString Name)
{
...
//---------------------------------------------------------------------------
AnsiString __fastcall TFMain_11011981::GetUnitName(PUnitRec recU)
{
    if (recU)
    {
        if (recU->names->Count == 1)
            ...
            return "_Unit" + AnsiString(recU->iniOrder);
    }
    return "";
}
//---------------------------------------------------------------------------
AnsiString __fastcall TFMain_11011981::GetUnitName(DWORD Adr)
{
    int     n;
    AnsiString  Result = "";
...
//---------------------------------------------------------------------------
void __fastcall TFMain_11011981::SetUnitName(PUnitRec recU, AnsiString name)
{
...
int __fastcall TFMain_11011981::EstimateProcSize(DWORD fromAdr)
{
    ...
    for (row = 0; row < outRows; row++)
    {
        ...
        if (b1 == 0xFF && (b2 & 0x38) == 0x20 && DisInfo.OpType[0] == otMEM && IsValidImageAdr(DisInfo.Offset)) //near absolute indirect jmp (Case)
        {
            ...
            //Analyze the gap based on the Table cTbl
            if (Adr <= lastMovAdr && lastMovAdr < jTblAdr) cTblAdr = lastMovAdr;
            //If there is cTblAdr, please skip this table
            BYTE CTab[256];
            if (cTblAdr)
            {
...
//---------------------------------------------------------------------------
int __fastcall TFMain_11011981::GetUnits2(AnsiString dprName)
{
    ...
    //Find the first instruction call = @InitExe
    n = 0;
    while (1)
    {
        instrLen = Disasm.Disassemble(Code + curPos, (__int64)curAdr, &DisInfo, 0);
    ...
    //Read call list (initialization process)
    int no = 1;
    for (num = 0;; num++)
    {
        instrLen = Disasm.Disassemble(Code + curPos, (__int64)curAdr, &DisInfo, 0);
...
//---------------------------------------------------------------------------
int __fastcall TFMain_11011981::GetUnits(AnsiString dprName)
{
    BYTE        len;
    TCHAR       *b, *e;
    ...
    for (i = 0, no = 1; i < unitsNum; i++, unitsPos += 8)
    {
        ...
        if (finAdr)
        {
            ...
            if (IsFlagSet(cfImport, pos))
            {
                ...
                {
                    e = strchr(b + 1, '@');
                    if (e) SetUnitName(recU, AnsiString(b + 1, e - b - 1));
                }
            }
        }
        Units->Add((void*)recU);
...
//---------------------------------------------------------------------------
int __fastcall TFMain_11011981::GetBCBUnits(AnsiString dprName)
{
            ...
            if (dd == 'hsup' && disInfo.OpType[0] == otIMM && disInfo.Immediate)
            {
                ...
                if (IsValidImageAdr(modTable))
                {
                    ...
                    for (n = 0; n < list->Count; n++)
                    {
                        ...
                        toAdr = StrToInt(AnsiString("$") + list->Strings[n]);
                        recU->finadr = 0;
                        recU->finSize = 0;
                        recU->iniadr = toAdr;
                        recU->iniSize = 0;
...
int __fastcall TFMain_11011981::IsValidCode(DWORD fromAdr)
{
    ...
    for (row = 0; row < outRows; row++)
    {
        ...
        if (op == OP_PUSH)
        {
            //If the first instruction is not push reg
        ...
        //First instruction rejection
        if (!row)
        {
            //Conversion instruction with ret c argument
        ...
        //If there is previously defined code in your location, please exit.
        ...
        if (b1 == 0xFF && (b2 & 0x38) == 0x20 && DisInfo.OpType[0] == otMEM && IsValidImageAdr(DisInfo.Offset)) //near absolute indirect jmp (Case)
        {
            ...
            */
            ...
            Pos = curPos + instrLen;
            Adr = curAdr + instrLen;
            //Table address - last 4 byte instruction
            jTblAdr = *((DWORD*)(Code + Pos - 4));
            //Analyze the gap according to the Table cTbl
            if (Adr <= lastMovAdr && lastMovAdr < jTblAdr) cTblAdr = lastMovAdr;
            //If there is cTblAdr, skip this table
            BYTE CTab[256];
            if (cTblAdr)
            {
                int CNum = jTblAdr - cTblAdr;
                        ...
                        if (Code[Pos] == 0xE9) //jmp Handle...
                        {
                            ...
                            if (recN)
                            {
                                ...
                                else if (recN->SameName("@HandleOnException"))
                                {
                                    //jmp HandleOnException
                                    Pos += instrLen1; Adr += instrLen1;
                                    //Checkbox cfETable, in order to correctly extract data
                                    SetFlag(cfETable, Pos);
                                    //dd num
                                    num = *((int*)(Code + Pos)); Pos += 4;
                                    if (Adr + 4 + 8 * num > lastAdr) lastAdr = Adr + 4 + 8 * num;
...
PMethodRec __fastcall TFMain_11011981::GetMethodInfo(PInfoRec rec, AnsiString name)
{
    if (rec && rec->vmtInfo->methods)
    {
        for (int m = 0; m < rec->vmtInfo->methods->Count; m++)
...
//---------------------------------------------------------------------------
//IntfTable attend, if the class is generated by an interface
/*
Interfaces
...
Automated Methods
    ...
    AnsiString method name, 4-bytes of flags, a pointer to a list of parameters,
...
void __fastcall TFMain_11011981::ScanAutoTable(DWORD Adr)
{
    ...
    AnsiString className = GetClsName(Adr);
    ...
    for (int i = 0; i < entryCount; i++)
    {
        ...
        AnsiString name = AnsiString((LPTSTR)(Code + posn), len);
        AnsiString procname = className + ".";
...
Initialization and Finalization
    ...
    uses a TTypeKind field to keep track of whether it is initializing a AnsiString,
    a record, an array, etc.
    An initialization table begins with the type kind (1 byte), followed by the
    type name as a short AnsiString, a 4-byte size of the data being initialized, a
...
void __fastcall TFMain_11011981::ScanInitTable(DWORD Adr)
{
    ...
    for (int i = 0; i < num; i++)
    {
        ...
        AnsiString typeName = AnsiString((LPTSTR)&Code[post], len);
...
void __fastcall TFMain_11011981::ScanFieldTable(DWORD Adr)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        AnsiString name = AnsiString((LPTSTR)(Code + pos), len); pos += len;
...
//---------------------------------------------------------------------------
void __fastcall TFMain_11011981::ScanMethodTable(DWORD adr, AnsiString className)
{
    BYTE        len;
    WORD        skipNext;
    DWORD       codeAdr;
    int         spos, pos;
    AnsiString      name, methodName;
    ...
    for (int n = 0; n < count; n++)
    {
        ...
        len = Code[pos]; pos++;
        name = AnsiString((LPTSTR)&Code[pos], len); pos += len;
...
{0x2F, "WMTCHARToItem", "WM_TCHARTOITEM"},
...
{0x102, "WMTCHAR", "WM_TCHAR"},
{0x103, "WMDeadTCHAR", "WM_DEADTCHAR"},
{0x104, "WMSysKeyDown", "WM_SYSKEYDOWN"},
{0x105, "WMSysKeyUp", "WM_SYSKEYUP"},
{0x106, "WMSysTCHAR", "WM_SYSTCHAR"},
{0x107, "WMSysDeadTCHAR", "WM_SYSDEADTCHAR"},
//{0x108, "?", "WM_KEYLAST"},
...
{0x120, "WMMenuTCHAR", "WM_MENUTCHAR"},
...
{0xB006, "CMDialogTCHAR", "CM_DIALOGTCHAR"},
...
void __fastcall TFMain_11011981::ScanDynamicTable(DWORD adr)
{
    ...
    for (int i = 0; i < num; i++)
    {
        ...
        else
        {
            ...
            if (_info)
            {
                AnsiString typname = _info->typname;
                if (typname != "")
                {
...
//---------------------------------------------------------------------------
//Return the "height" of the class (with 0 parent classes)
...
//---------------------------------------------------------------------------
PMethodRec __fastcall TFMain_11011981::GetMethodInfo(DWORD adr, TCHAR kind, int methodOfs)
{
...
//---------------------------------------------------------------------------
//Return the universal parent type of Name1, Name2
AnsiString __fastcall TFMain_11011981::GetCommonType(AnsiString Name1, AnsiString Name2)
{
    ...
    if (!adr1 || !adr2)
    {
    	...
    	if ((SameText(Name1, "String") && SameText(Name2, "AnsiString")) ||
        	(SameText(Name1, "String") && SameText(Name2, "AnsiString"))) return "AnsiString";
        //Text - TTextRec
...
//----Highlighting-----------------------------------------------------------
	if (InitHighlight())
    {
        ...
        for (n = 0; n < DelphiThemesCount; n++)
        {
            ...
            mi->Caption = AnsiString((TCHAR *)buf);
            mi->Tag = n;
...
//---------------------------------------------------------------------------
AnsiString __fastcall GetFilenameFromLink(AnsiString LinkName)
{
    AnsiString          result = "";
    IPersistFile*   ppf;
    IShellLink*     pshl;
    ...
    //Load Shortcut
    LPWSTR temp = new WCHAR[MAX_PATH];
    ...
    //Get Object name
    LPTSTR targetName = new TCHAR[MAX_PATH];
    pshl->GetPath(targetName, MAX_PATH, &wfd, 0);
    result = AnsiString(targetName);
    delete[] targetName;
...
void __fastcall TFMain_11011981::ScanImports()
{
    ...
    //Attempt to scan the address interval of unnamed modules by importing function names
    for (int m = 0; m < UnitsNum; m++)
    {
        ...
        if (!recU->names->Count)
        {
            ...
            if (totcnt)
            {
                ...
                //If all imports are in one unit, it means it is.
                if (num == 1)
                {
                ...
                //If there are multiple, please try searching for the cfprocstart process (if available)
                else
                {
                    for (int k = 0; k < KnowledgeBase.ModuleCount; k++)
                    {
...
*/
//---------------------------------------------------------------------------
AnsiString __fastcall TFMain_11011981::MakeComment(PPICODE Picode)
{
    bool	vmt;
    DWORD	vmtAdr;
	AnsiString	comment = "";
...
//XXXXXXXX - adr
//F - flags
int __fastcall TFMain_11011981::AddAsmLine(DWORD Adr, AnsiString text, BYTE Flags)
{
    AnsiString _line = " " + Val2Str8(Adr) + "        " + text + " ";
    if (Flags & 1) _line[1] = '>';
    if (Flags & 8) _line[10] = '>';
...
void __fastcall TFMain_11011981::ShowCode(DWORD fromAdr, int SelectedIdx, int XrefIdx, int topIdx)
{
    ...
    while (row < outRows)
    {
        ...
        if (IsFlagSet(cfETable, curPos))
        {
            //dd num
            num = *((int*)(Code + curPos));
            wid = AddAsmLine(curAdr, "dd          " + AnsiString(num), flags); row++;
            ...
            */
            ...
            if (cTblAdr)
            {
                ...
                for (int k = 0; k < CNum; k++)
                {
                    BYTE db = Code[Pos];
                    CTab[k] = db;
                    wid = AddAsmLine(Adr, "db          " + AnsiString(db), 0x22); row++;
                    if (wid > maxwid) maxwid = wid;
                    Pos++; Adr++;
                }
            }
...
//        dd num
//Дале?таблиц?из num записе?вида
//        dd offset ExceptionInfo
//        dd offset ExceptionProc
//----------------------------------
        if (b1 == 0x68)		//try block	(push loc_TryBeg)
        ...
        AnsiString namei = "", comment = "", name, pname, type, ptype;
        if (NameInside)
        {
...
void __fastcall TFMain_11011981::AnalyzeMethodTable(int Pass, DWORD Adr, const bool* Terminated)
{
    ...
    PInfoRec    recN;
    AnsiString      paramName, methodName;
    DWORD vmtAdr = Adr - cVmtSelfPtr;
    DWORD methodAdr = *((DWORD*)(Code + Adr2Pos(vmtAdr) + cVmtMethodTable));
    ...
    for (int n = 0; n < count && !*Terminated; n++)
    {
        ...
        sLen = Code[pos + 6];
        methodName = AnsiString((LPTSTR)(Code + pos + 7), sLen);
    ...
    if (DelphiVersion >= 2010)
    {
        ...
        for (int n = 0; n < exCount && !*Terminated; n++)
        {
            ...
            procPos = Adr2Pos(procAdr);
            sLen = Code[pos];
            methodName = AnsiString((LPTSTR)(Code + pos + 1), sLen); pos += sLen + 1;
            ...
            if (pos - Adr2Pos(methodEntry) < skipNext)
            {
                ...
                for (int m = 0; m < paramCount && !*Terminated; m++)
                {
                    ...
                    sLen = Code[pos];
                    paramName = AnsiString((LPTSTR)(Code + pos + 1), sLen); pos += sLen + 1;
                    //AttrData
                    dw = *((WORD*)(Code + pos));
                    pos += dw;//ATR!!
...
void __fastcall TFMain_11011981::AnalyzeDynamicTable(int Pass, DWORD Adr, const bool* Terminated)
{
	...
	AnsiString clsName = GetClsName(Adr);
            ...
            while (pAdr&& !*Terminated)
            {
                ...
                if (recN1 && recN1->vmtInfo && recN1->vmtInfo->methods)
                {
                    for (int m = 0; m < recN1->vmtInfo->methods->Count; m++)
                    {
                        ...
                        {
                            AnsiString procName = recN->GetName();
...
//---------------------------------------------------------------------------
int __fastcall TFMain_11011981::CodeGetTargetAdr(AnsiString Line, DWORD* trgAdr)
{
    TCHAR	*s, *p, c;
    int		n, wid, instrlen;
    DWORD	adr, targetAdr;
...
//---------------------------------------------------------------------------
//May be Plugin!!!
AnsiString __fastcall sub_004AFB28(BYTE* AStr)
{
    Integer   _n, _num;
    Byte      _b, _b1, _b2, _m;
    AnsiString    _result;
...
void __fastcall TFMain_11011981::GoToAddress()
{
    ...
    if (sAdr != "")
    {
        ...
        {
            pos = Adr2Pos(gotoAdr);
            //If imported - not displayed
            if (IsFlagSet(cfImport, pos)) return;
            //Where is the search address
            while (pos >= 0)
            {
                //Program Start
                if (IsFlagSet(cfProcStart, pos))
...
void __fastcall TFMain_11011981::NamePosition()
{
    ...
    DWORD 		adr, nameAdr;
    PInfoRec 	recN;
    AnsiString 		line, text = "", sNameType, newName, newType;
...
//---------------------------------------------------------------------------
TTreeNode* __fastcall TFMain_11011981::GetNodeByName(AnsiString AName)
{
...
//---------------------------------------------------------------------------
TTreeNode* __fastcall TFMain_11011981::FindTreeNodeByName(const AnsiString& name)
{
...
//---------------------------------------------------------------------------
void __fastcall TFMain_11011981::AddTreeNodeWithName(TTreeNode* node, const AnsiString& name)
{
...
void __fastcall TFMain_11011981::ShowClassViewer(DWORD VmtAdr)
{
    ...
    AnsiString      SelName = GetClsName(VmtAdr), line, name;
    TTreeNode*  selNode = 0;
    TTreeNode*  node = 0;
    PInfoRec    recN;
    PMethodRec  recM;
    ...
    for (int n = 0;; n++)
    {
        ...
        if (!parentAdr)
        {
            while (n >= 0)
            {
                ...
                AnsiString text = className + " #" + Val2Str8(adr) + " Sz=" + Val2Str0(size);
                ...
                if (intfsNum)
                {
                    ...
                    {
                        AnsiString item = tmpList->Strings[m];
...
int __fastcall TFMain_11011981::LoadAutoTable(DWORD adr, TStringList* dstList)
{
    ...
    if (recN && recN->vmtInfo && recN->vmtInfo->methods)
    {
        for (int n = 0; n < recN->vmtInfo->methods->Count; n++)
        {
            ...
            if (recM->kind == 'A')
            {
                AnsiString line = "A" + Val2Str4(recM->id) + " #" + Val2Str8(recM->address) + " " + recM->name;
                dstList->Add(line);
            }
        }
    }
...
int __fastcall TFMain_11011981::LoadMethodTable(DWORD adr, TStringList* dstList)
{
    ...
    if (recN && recN->vmtInfo && recN->vmtInfo->methods)
    {
        for (int n = 0; n < recN->vmtInfo->methods->Count; n++)
        {
            ...
            if (recM->kind == 'M')
            {
                AnsiString line = "#" + Val2Str8(recM->address) + " " + recM->name;
                dstList->Add(line);
            }
        }
    }
...
int __fastcall TFMain_11011981::LoadDynamicTable(DWORD adr, TStringList* dstList)
{
    ...
    if (recN && recN->vmtInfo && recN->vmtInfo->methods)
    {
        for (int n = 0; n < recN->vmtInfo->methods->Count; n++)
        {
            ...
            if (recM->kind == 'D')
            {
                AnsiString line = "D" + Val2Str4(recM->id) + " #" + Val2Str8(recM->address) + " " + recM->name;
                dstList->Add(line);
            }
        }
        dstList->Sort();
...
void __fastcall TFMain_11011981::miViewProtoClick(TObject *Sender)
{
    ...
    if (DisInfo.Call)
    {
        ...
        //Address not specified, please try PI code
        else
        {
        	recN = GetInfoRec(Adr);
            if (recN && recN->picode && IsValidCodeAdr(recN->picode->Ofs.Address))
...
void __fastcall TFMain_11011981::tvClassesDblClick(TObject *Sender)
{
    ...
    if (node)
    {
        ...
        if (line.Pos(":"))
        {
            AnsiString typeName = ExtractType(line);
            //If the type is set to Unit.TypeName
...
{
    AnsiString sName = InputDialogExec("Enter Name of Type", "Name:", "");
...
void __fastcall TFMain_11011981::miEditClassClick(TObject *Sender)
{
	if (ActiveControl == tvClassesShort)
    {
        ...
        if (node)
        {
			...
			if (!node->Text.Pos("#"))
			{
				LPTSTR text = TransformPChar(CP_ACP, node->Text.c_str(), -1);
				sscanf(text, "%lX", &FieldOfs);
				delete[] text;
			}
        	while (node)
            {
...
void __fastcall TFMain_11011981::ShowDfm(TDfm* dfm)
{
    ...
    if (dfm->Form)
    {
        ...
        if (recU)
        {
            ...
            dfm->Form->Caption = AnsiString(StringBuf, stringLen);
        }
        dfm->Open = 2;
        dfm->Form->Show();
...
//---------------------------------------------------------------------------
void __fastcall TFMain_11011981::FindText(AnsiString Text)
{
    int         n, pos, idx = -1;
    AnsiString      line, findText, msg;
    TTreeNode*  node;
    TTreeView*  tv;

    if (Text == "") return;
    
    msg = "Search AnsiString \"" + Text + "\" not found";
...
{
    int         n, m, pos, version; AnsiString str, filename, ident;
    TMenuItem   *item;
    TIniFile    *iniFile;
    TFont       *_font;
	Forms::TMonitor    *_monitor;

    iniFile = new TIniFile(ChangeFileExt(Application->ExeName, ".ini"));

    _font = new TFont;
    _font->Name = iniFile->ReadString("Settings", "FontName", "Fixedsys");
    _font->Charset = iniFile->ReadInteger("Settings", "FontTCHARset", 1);
...
void __fastcall TFMain_11011981::IniFileWrite()
{
    ...
    iniFile->WriteInteger("Settings", "FontTCHARset", lbCode->Font->Charset);
    ...
    //Delete all
    int n; AnsiString ident;
    for (n = 0; n < 8; n++) iniFile->DeleteKey("Recent Executable Files", "File" + AnsiString(n + 1));
    for (n = 9; n < 17; n++) iniFile->DeleteKey("Recent Executable Files", "File" + AnsiString(n - 8));

    //Fill
    for (n = 0; n < 8; n++)
    {
        TMenuItem *item = miMRF->Items[n];
        if (item->Visible && item->Enabled) iniFile->WriteString("Recent Executable Files", "File" + AnsiString(n + 1), "\"" + item->Caption + "\"," + AnsiString(item->Tag));
    }
    for (n = 9; n < 17; n++)
    {
        TMenuItem *item = miMRF->Items[n];
        if (item->Visible && item->Enabled) iniFile->WriteString("Recent Project Files", "File" + AnsiString(n - 8), "\"" + item->Caption + "\"");
    }
...
//LOAD EXE AND IDP
//---------------------------------------------------------------------------
bool __fastcall TFMain_11011981::IsExe(AnsiString FileName)
{
...
//---------------------------------------------------------------------------
bool __fastcall TFMain_11011981::IsIdp(AnsiString FileName)
{
    TCHAR    buf[13];
...
//---------------------------------------------------------------------------
void __fastcall TFMain_11011981::LoadFile(AnsiString FileName, int version)
{
    if (ProjectModified)
    {
...
//
void __fastcall TFMain_11011981::DoOpenDelphiFile(int version, AnsiString FileName, bool loadExp, bool loadImp)
{
    if (ProjectModified)
    {
		int res = Application->MessageBox(L"Save active Project?", L"Confirmation", MB_YESNOCANCEL);
...
//---------------------------------------------------------------------------
void __fastcall TFMain_11011981::LoadDelphiFile1(AnsiString FileName, int version, bool loadExp, bool loadImp)
{
    int			pos, ver;
    AnsiString		dprName, KBFileName, msg;
    ...
    {
        Screen->Cursor = crDefault;
        ShowMessage("Cannot open Knowledge Base file " + AnsiString(KBFileName) + " (may be incorrect Version)");
        CleanProject();
        return;
    }
    ...
    if (DelphiVersion <= 2010)
        Caption = "Interactive Delphi Reconstructor by crypto: " + SourceFile + " (Delphi-" + AnsiString(DelphiVersion) + ")";
    else
        Caption = "Interactive Delphi Reconstructor by crypto: " + SourceFile + " (Delphi-XE" + AnsiString(DelphiVersion - 2010) + ")";
    ...
    bEP->Enabled = true;
    //Disable menu items when downloading files
    miLoadFile->Enabled = false;
    miOpenProject->Enabled = false;
    miMRF->Enabled = false;
    miSaveProject->Enabled = false;
...
bool __fastcall TFMain_11011981::ImportsValid(DWORD ImpRVA, DWORD ImpSize)
{
    if (ImpRVA || ImpSize)
    {
        ...
        while (1)
        {
            ...
            int NameLength = strlen((LPTSTR)(Image + Adr2Pos(ImportDescriptor.Name + ImageBase)));
...
int __fastcall TFMain_11011981::LoadImage(FILE* f, bool loadExp, bool loadImp)
{
    ...
    DWORD       	        DataEnd, Items;
    AnsiString      	        moduleName, modName, sEP;
    AnsiString      	        impFuncName;
    IMAGE_DOS_HEADER        DosHeader;
    IMAGE_NT_HEADERS        NTHeaders;
    ...
    for (i = 0; i < SectionsNum; i++)
    {
        ...
        segInfo->Name = AnsiString(segname);
    ...
    if (evalEP + CodeBase != NTHeaders.OptionalHeader.AddressOfEntryPoint + ImageBase)
    {
		...
		LPWSTR wmsg = TransformPWideChar(CP_ACP, msg, -1);
		if (Application->MessageBox(wmsg, L"Confirmation", MB_YESNO) == IDYES)
		{
    ...
    if (loadExp)
    {
        ...
        if (ExpRVA)
        {
            ...
            for (i = 0; i < ExpNum; i++)
            {
                ...
                NameLength = strlen((LPTSTR)(Image + Adr2Pos(dp + ImageBase)));
                recE->name = AnsiString((LPTSTR)(Image + Adr2Pos(dp + ImageBase)), NameLength);
    ...
    if (loadImp && (ImpRVA || ImpSize))
    {
        ...
        else
        {
            ...
            while (1)
            {
                ...
                NameLength = strlen((LPTSTR)(Image + Adr2Pos(ImportDescriptor.Name + ImageBase)));
                moduleName = AnsiString((LPTSTR)(Image + Adr2Pos(ImportDescriptor.Name + ImageBase)), NameLength);
                ...
                while (1)
                {
                    ...
                    if (ThunkValue & 0x80000000)
                    {
                        ...
                        //if (hLib) fnProc = (DWORD)GetProcAddress(hLib, (LPTSTR)Hint);

                        //Addresse get from FirstThunk only
                        //recI->name = modName + "." + AnsiString(Hint);
                        recI->name = AnsiString(Hint);
                    }
                    else
                    {
                        // by name
                        Hint = *((WORD*)(Image + Adr2Pos(ThunkValue + ImageBase)));
                        NameLength = lstrlen((LPTSTR)(Image + Adr2Pos(ThunkValue + 2 + ImageBase)));
                        impFuncName = AnsiString((LPTSTR)(Image + Adr2Pos(ThunkValue + 2 + ImageBase)), NameLength);
...
//---------------------------------------------------------------------------
void __fastcall TFMain_11011981::DoOpenProjectFile(AnsiString FileName)
{
    TCHAR	*buf;
...
//---------------------------------------------------------------------------
void __fastcall TFMain_11011981::ReadNode(TStream* stream, TTreeNode* node, LPTSTR buf)
{
    //Count
    int itemsCount;
...
void __fastcall TFMain_11011981::OpenProject(AnsiString FileName)
{
    ...
    for (n = 0; n < num; n++)
    {
        ...
        segInfo->Name = AnsiString(buf, len);
        SegmentList->Add((void*)segInfo);
    }
    ...
    for (n = 0; n < bssCnt; n++)
    {
        inStream->Read(&len, sizeof(len));
        inStream->Read(buf, len);
        AnsiString _adr = AnsiString(buf, len);
        inStream->Read(&kind, sizeof(kind));
    ...
    for (n = 0; n < UnitsNum; n++)
    {
        ...
        for (u = 0; u < namesNum; u++)
        {
            inStream->Read(&len, sizeof(len));
            inStream->Read(buf, len);
            SetUnitName(recU, AnsiString(buf, len));
        }
        Units->Add((void*)recU);
    }
    UnitSortField = 0;
    ...
    for (n = 0; n < num; n++)
    {
        ...
        inStream->Read(&len, sizeof(len));
        inStream->Read(buf, len);
        dfm->ResName = AnsiString(buf, len);
        //Name
        inStream->Read(&len, sizeof(len));
        inStream->Read(buf, len);
        dfm->Name = AnsiString(buf, len);
        //ClassName
        inStream->Read(&len, sizeof(len));
        inStream->Read(buf, len);
        dfm->ClassName = AnsiString(buf, len);
        //MemStream
        ...
        for (m = 0; m < evnum; m++)
        {
            ...
            inStream->Read(&len, sizeof(len));
            inStream->Read(buf, len);
            eInfo->EventName = AnsiString(buf, len);
            //ProcName
            inStream->Read(&len, sizeof(len));
            inStream->Read(buf, len);
            eInfo->ProcName = AnsiString(buf, len);
        ...
        if (cnum)
        {
            ...
            for (m = 0; m < cnum; m++)
            {
                ...
                inStream->Read(&len, sizeof(len));
                inStream->Read(buf, len);
                cInfo->Name = AnsiString(buf, len);
                //ClassName
                inStream->Read(&len, sizeof(len));
                inStream->Read(buf, len);
                cInfo->ClassName = AnsiString(buf, len);
                //Events
    ...
    for (n = 0; n < num; n++)
    {
        inStream->Read(&len, sizeof(len));
        inStream->Read(buf, len);
        ResInfo->Aliases->Add(AnsiString(buf, len));
    }
    InitAliases(false);
    tsForms->Enabled = (lbForms->Items->Count > 0);
...
//---------------------------------------------------------------------------
void __fastcall TFMain_11011981::SaveProject(AnsiString FileName)
{
    ...
    try
    {
        ...
        FProgressBar->StartProgress("Writing Infos Objects (number = " + AnsiString(infosCnt) + ")...", "", TotalSize / 4096);
        MaxBufLen = 0;
        BYTE kind;
        try
        {
        ...
        //Units
        num = UnitsNum;
        FProgressBar->StartProgress("Writing Units (number = "+AnsiString(num)+")...", "", num);
        outStream->Write(&num, sizeof(num));
        for (n = 0; n < num; n++)
        {
            FProgressBar->pb->StepIt();
        ...
        FProgressBar->StartProgress("Writing Types (number = "+AnsiString(num)+")...", "", num);
        outStream->Write(&num, sizeof(num));
        for (n = 0; n < num; n++)
        {
            FProgressBar->pb->StepIt();
        ...
        FProgressBar->StartProgress("Writing Forms (number = "+AnsiString(num)+")...", "", num);
        outStream->Write(&num, sizeof(num));
        for (n = 0; n < num; n++)
        {
            FProgressBar->pb->StepIt();
        ...
        FProgressBar->StartProgress("Writing Aliases  (number = "+AnsiString(num)+")...", "", num);
        outStream->Write(&num, sizeof(num));
        for (n = 0; n < num; n++)
        {
            FProgressBar->pb->StepIt();
        ...
        FProgressBar->StartProgress("Writing Code History Items (number = "+AnsiString(CodeHistorySize)+")...", "", CodeHistorySize);
        for (n = 0; n < CodeHistorySize; n++)
        {
            FProgressBar->pb->StepIt();
            Application->ProcessMessages();
		...
		if (num && Application->MessageBox(L"Save full Tree of Classes?", L"Warning", MB_YESNO) == IDYES)
        {
            ...
            if (num)
            {
                FProgressBar->StartProgress("Writing ClassViewer Tree Nodes (number = "+AnsiString(num)+")...", "", num);
...
//---------------------------------------------------------------------------
void __fastcall TFMain_11011981::AddExe2MRF(AnsiString FileName)
{
    int     n, m;
    for (n = 0; n < 8; n++)
    {
...
//---------------------------------------------------------------------------
void __fastcall TFMain_11011981::AddIdp2MRF(AnsiString FileName)
{
    int     n, m;
    for (n = 9; n < 17; n++)
    {
...
void __fastcall TFMain_11011981::miKBTypeInfoClick(TObject *Sender)
{
    ...
    MProcInfo   pInfo;
    MTypeInfo   tInfo;
    AnsiString      typeName, className, propName, sName;
...
void __fastcall TFMain_11011981::EditFunction(DWORD Adr)
{
    ...
    PMethodRec  recM;
    PARGINFO 	argInfo;
    AnsiString      line, name, typeDef, item, className, procName;
    TCHAR        buf[1024];
        ...
        if (FEditFunctionDlg_11011981->ShowModal() == mrOk)
        {
            ...
            if (0)//recN->info.procInfo->locals)
            {
                ...
                for (n = 0; n < cnt; n++)
                {
                    ...
                    p = strtok(0, " :");
                    if (stricmp(p, "?"))
                        name = AnsiString(p).Trim();
                    else
                        name = "";
                    //type
                    p = strtok(0, " ");
                    if (stricmp(p, "?"))
                        typeDef = AnsiString(p).Trim();
                    else
                        typeDef = "";
        ...
        if (IsFlagSet(cfProcStart, n) && !IsFlagSet(cfEmbedded, n))
        {
            ...
            if (recN)
            {
                if (adr != EP)
                {
                    ...
                    if (recU)
                    {
                        AnsiString moduleName = GetUnitName(recU);
                        if (adr == recU->iniadr)
                            procName = "Initialization";
...
{
    AnsiString idcName = "", idcTemplate = "";
    if (SourceFile != "")
    {
    ...
    for (pos = 0, curSize = 0; pos < TotalSize; pos++)
    {
        ...
        if (IsFlagSet(cfRTTI, pos))
        {
            ...
            if (recU->names->Count == 1)
                ...
                idcGen->unitName = ".Unit" + AnsiString(recU->iniOrder);
...
void __fastcall TFMain_11011981::pmUnitsPopup(TObject *Sender)
{
    ...
    AnsiString item = lbUnits->Items->Strings[lbUnits->ItemIndex];
    DWORD adr;
    sscanf(item.c_str() + 1, "%lX", &adr);
    PUnitRec recU = GetUnit(adr);
      ...
      int Index, TRect &Rect, TOwnerDrawState State)
{
    ...
    TListBox    *lb;
    TCanvas     *canvas;
    AnsiString      text, _item, _comment;
    PInfoRec    _recN;
    DISINFO     _disInfo;
    ...
    if (Index < lb->Count)
    {
        ...
        _item = text.SubString(2, 8); _adr = StrToInt(AnsiString("$") + _item);
        //loop or loc
        if (_f & 5)
        ...
        _item = AnsiString(_disInfo.Mnem); _len = _item.Length();
        if (!_disInfo.Float)
            _color = TColor(0);
        else
            ...
            if (_disInfo.OpNum)
            {
                ...
                for (n = 0; n < _disInfo.OpNum; n++)
                {
                    ...
                    else if (_disInfo.OpType[n] == otMEM)
                    {
                        if (_disInfo.OpSize)
                        {
                            _item = AnsiString(_disInfo.sSize) + " ptr ";
                            DrawOneItem(_item, canvas, Rect, TColor(0), flags);
                        }
                        if (_disInfo.SegPrefix != -1)
                        {
                            _item = AnsiString(SegRegTab[_disInfo.SegPrefix]);
                            DrawOneItem(_item, canvas, Rect, TColor(0x0000B0), flags);
                        ...
                        if (ib)
                        {
                            ...
                            if (_disInfo.IndxReg != -1)
                            {
                                ...
                                {
                                    DrawOneItem("*", canvas, Rect, TColor(0), flags);
                                    _item = AnsiString(_disInfo.Scale);
                                    DrawOneItem(_item, canvas, Rect, TColor(0xFF8080), flags);
                                }
                            }
                            if (_offset)
        ...
        {
            _item += AnsiString(_disInfo.sSize[0]);
            DrawOneItem(_item, canvas, Rect, _color, flags);
        }
        //Comment (light gray)
        if (_cPos)
...
//---------------------------------------------------------------------------
void __fastcall TFMain_11011981::OutputLine(FILE* OutF, BYTE flags, DWORD Adr, AnsiString Content)
{
    //Ouput comments
    if (flags & 0x10)
    {
        TCHAR *p = strchr(Content.c_str(), ';');
...
//---------------------------------------------------------------------------
void __fastcall TFMain_11011981::OutputCode(FILE* outF, DWORD fromAdr, AnsiString prototype, bool onlyComments)
{
    BYTE        op, flags;
    ...
    while (row < outRows)
    {
        ...
        if (IsFlagSet(cfETable, curPos))
        {
            //dd num
            num = *((int*)(Code + curPos));
            OutputLine(outF, flags, curAdr, "dd          " + AnsiString(num)); row++;
            curPos += 4; curAdr += 4;
            ...
            */
            ...
            if (cTblAdr)
            {
                ...
                for (int k = 0; k < CNum; k++)
                {
                    BYTE db = Code[Pos];
                    CTab[k] = db;
                    OutputLine(outF, flags, curAdr, "db          " + AnsiString(db)); row++;
                    Pos++; Adr++;
                }
            }
            //Check transitions by negative register values (in Delphi 2009)
...
//@1:     jmp HandleOnException
//        dd num
//The next one is the num view record table
//        dd offset ExceptionInfo
//        dd offset ExceptionProc
//----------------------------------
        if (b1 == 0x68)		//try block	(push loc_TryBeg)
...
void __fastcall TFMain_11011981::wm_dropFiles(TWMDropFiles& msg)
{
    ...
    try
    {
        ...
        {
            AnsiString droppedFile = fc->Files[i];
            AnsiString ext = ExtractFileExt(droppedFile).LowerCase();
...
{
	LPTSTR text = TransformPChar(CP_ACP, Application->HelpFile.c_str(), -1);
	ShellExecute(Handle, "open", text, 0, 0, 1);
	delete[] text;
}
//---------------------------------------------------------------------------
...
void __fastcall TFMain_11011981::FormCloseQuery(TObject *Sender, bool &CanClose)
{
    ...
    if (AnalyzeThread)
    {
        ...
        AnsiString sbtext0 = FProgressBar->sb->Panels->Items[0]->Text;
        AnsiString sbtext1 = FProgressBar->sb->Panels->Items[1]->Text;
...
//---------------------------------------------------------------------------
PFIELDINFO __fastcall TFMain_11011981::GetField(AnsiString TypeName, int Offset, bool* vmt, DWORD* vmtAdr, AnsiString prefix)
{
        ...
        if (tInfo->Fields)
        {
            ...
            for (n = 0; n < tInfo->FieldsNum; n++)
            {
                ...
                if (Offset >= Ofs1 && Offset < Ofs2)
                {
                    ...
                    fInfo->xrefs = 0;
                    Len = *((WORD*)p); p += 2;
                    fInfo->Name = AnsiString((LPTSTR)p, Len); p += Len + 1;
                    Len = *((WORD*)p); p += 2;
                    fInfo->Type = TrimTypeName(AnsiString((LPTSTR)p, Len));
                    return fInfo;
                }
            }
            return 0;
...
//---------------------------------------------------------------------------
PFIELDINFO __fastcall TFMain_11011981::AddField(DWORD ProcAdr, int ProcOfs, AnsiString TypeName, BYTE Scope, int Offset, int Case, AnsiString Name, AnsiString Type)
{
...
void __fastcall TFMain_11011981::InitAliases(bool find)
{
    ...
    {
        AnsiString item = ResInfo->Aliases->Strings[n];
        if (item.Pos("="))
        {
            LPWSTR p = AnsiLastChar(item);
    ...
    for (int n = 0;; n++)
    {
        ...
        if (RegClasses[n].ClassName)
            cbAliases->Items->Add(AnsiString(RegClasses[n].ClassName));
    }
...
void __fastcall TFMain_11011981::lbAliasesDblClick(TObject *Sender)
{
    lClassName->Caption = "";
    cbAliases->Text = "";
    AnsiString item = lbAliases->Items->Strings[lbAliases->ItemIndex];
    int pos = item.Pos("=");
    if (pos)
    {
...
void __fastcall TFMain_11011981::miCopyListClick(TObject *Sender)
{
    ...
    FILE        *outFile;
    TStringList *tmpList;
    AnsiString      moduleName, importName;
    WORD        uses[128];
    ...
    if (SaveDlg->Execute())
    {
		...
		LPTSTR fname = TransformPChar(CP_ACP, SaveDlg->FileName.c_str(), -1);
...
void __fastcall TFMain_11011981::FillClassViewerOne(int n, TStringList* tmpList, const bool* terminated)
{
    ...
    AnsiString      className, nodeTextParent, nodeText, line, name;
    PInfoRec    recN;
    PMethodRec  recM;
    PVmtListRec recV;
    DISINFO     disInfo;
...
//---------------------------------------------------------------------------
TTreeNode* __fastcall TFMain_11011981::AddClassTreeNode(TTreeNode* node, AnsiString nodeText)
{
    TTreeNode* newNode = 0;
    if (!node)  //Root
...
void __fastcall TFMain_11011981::miSaveDelphiProjectClick(TObject *Sender)
{
    ...
    AnsiString          curDir, DelphiProjectPath;
    AnsiString          unitName, className, parentName, fieldName, typeName;
    AnsiString          procName, formName, dfmName, line, uName;
    ...
    for (n = 0; n < UnitsNum; n++)
    {
        ...
        for (adr = recU->fromAdr; adr < recU->toAdr; adr++)
        {
            ...
            case ikDynArray:
                ...
                intBodyLines->Add(AnsiString(StringBuf, len));
                break;
            //class
            case ikVMT:
                typePresent = true;
                ...
                if (dfm)
                {
                    ...
                    unitsList->Add(AnsiString(StringBuf, len));
                }
                ...
                for (m = 0; m < num; m++)
                {
                    ...
                    if (_isForm && dfm && dfm->IsFormComponent(fieldName))
                        publishedList->Add(AnsiString(StringBuf, len));
                    else
                        publicList->Add(AnsiString(StringBuf, len));
                ...
                for (m = 0; m < num; m++)
                {
                    ...
                    {
                        procName += AnsiString(" message ") + _info->msgname + ";";
                    }
                    else
                        procName += " dynamic;";
                ...
                for (adr1 = recU->fromAdr; adr1 < recU->toAdr; adr1++)
                {
                    ...
                    if (recN->kind == ikConstructor || (recN->procInfo->flags & PF_METHOD))
                    {
                        ...
                        if (!procName.Pos(":?"))
                            ...
                            len = sprintf(StringBuf, "    //%s//%08lX", procName.c_str(), adr1);
                        if (intBodyLines->IndexOf(AnsiString(StringBuf, len)) == -1)
                            intBodyLines->Add(AnsiString(StringBuf, len));
                    }
                }
        ...
        for (adr = recU->fromAdr; adr < recU->toAdr; adr++)
        {
            ...
            if (intBodyLines->IndexOf(AnsiString(StringBuf, len)) != -1) continue;
...
//---------------------------------------------------------------------------
void __fastcall TFMain_11011981::CopyAddress(AnsiString line, int ofs, int bytes)
{
    TCHAR    buf[9], *p = buf;
	...
	*p = 0;
	LPWSTR wbuf = TransformPWideChar(CP_ACP, buf, -1);
...
void __fastcall TFMain_11011981::pmCodePopup(TObject *Sender)
{
    ...
    if (ActiveControl == lbCode)
    {
        if (lbCode->ItemIndex <= 0) return;
		DWORD   adr;
		LPTSTR text = TransformPChar(CP_ACP, lbCode->Items->Strings[lbCode->ItemIndex].c_str(), -1);
		sscanf(text + 2, "%lX", &adr);
		delete[] text;
...
void __fastcall TFMain_11011981::lbCodeClick(TObject *Sender)
{
    ...
    AnsiString prevItem = SelectedAsmItem;
    SelectedAsmItem = "";
    AnsiString text = lbCode->Items->Strings[lbCode->ItemIndex];
...
{
    AnsiString PluginsPath = AppDir + "Plugins";
...
void __fastcall TFMain_11011981::miSwitchSkipFlagClick(TObject *Sender)
{
    ...
    if (lbCode->SelCount > 0)
    {
        for (int n = 0; n < lbCode->Count; n++)
        {
            ...
            {
				LPTSTR text = TransformPChar(CP_ACP, lbCode->Items->Strings[n].c_str(), -1);
				sscanf(text + 2, "%lX", &_adr);
				delete[] text;
...
void __fastcall TFMain_11011981::lbSourceCodeClick(TObject *Sender)
{
    ...
    AnsiString prevItem = SelectedSourceItem;
    SelectedSourceItem = "";
    AnsiString text = lbSourceCode->Items->Strings[lbSourceCode->ItemIndex];
    int textLen = text.Length();
...
void __fastcall TFMain_11011981::miSetlvartypeClick(TObject *Sender)
{
    ...
    {
        AnsiString ftype = InputDialogExec("Enter Type of " + SelectedSourceItem, "Type", pLocInfo->TypeDef).Trim();
...
void __fastcall TFMain_11011981::miHiewGeneratorClick(TObject *Sender)
{
    int     _posNamet;
    AnsiString  procName;
    AnsiString  nametName = "";
    AnsiString  SourceFileNamet = "";
    AnsiString moduleName = "";
...
//TerminatorX code BEGIN
void __fastcall TFMain_11011981::GoToAddressMap(AnsiString cmdAdr)
{
    int             pos;
    DWORD           gotoAdr;
    AnsiString          sAdr;
...
//---------------------------------------------------------------------------
void __fastcall TFMain_11011981::ExploreAdrMap(AnsiString cmdAdr)
{
    int         size;
    DWORD       viewAdr;
    AnsiString      text = "", sAdr;
...
{
    AnsiString  procName, moduleName;
...
void __fastcall TFMain_11011981::ExpGotoAddressMap(bool FlagGo)
{
    ...
    for (int d = 0; d < lstMap->Items->Count; d++)
    {
        if(lstMap->Selected[d])
        {
			...
			int v = 0;
			if (TryStrToInt(("$" + strhexs11.SubString(1,8)), v))
            {
                if (FlagGo)
                {
