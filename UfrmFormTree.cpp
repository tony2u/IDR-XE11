...
//---------------------------------------------------------------------------
void __fastcall TIdrDfmFormTree_11011981::AddEventsToNode(AnsiString compName, TTreeNode* dstNode, TList* evList)
{
    ...
    : TForm(Owner)
{
    ...
    if (compCnt > 1)
        Caption = frmOwner->Name + " (" + AnsiString(compCnt) + " components)";
    else
    ...
    for (int i = 0; i < compCnt; i++)
    {
        ...
        AnsiString tnCaption = MakeNodeCaption(currComp);
        ...
        if (currComp->HasParent())
        {
			...
			AnsiString name = TransformUString(CP_ACP, parCompo->Name.c_str(), -1);
			AnsiString parName = parCompo ? name : AnsiString("N/A");
            AnsiString ownName = currComp->Owner->Name;
            ...
            if (!parentNode && parCompo)
            {
                ...
                AnsiString parName2    = parCompo2->Name;
    ...
    if (externalParent)
    {
        Application->MessageBox(String("Form has some component classes defined in external modules:\r\n"
            + externalNames + ".\r\n\r\nVisualization of these components is not yet implemented").c_str(), (LPWSTR)"Warning", MB_OK | MB_ICONWARNING);
    }
}
//---------------------------------------------------------------------------
AnsiString TIdrDfmFormTree_11011981::MakeNodeCaption(TComponent* curCompo)
{
    AnsiString className = curCompo->ClassName();
...
//---------------------------------------------------------------------------
AnsiString TIdrDfmFormTree_11011981::MakeNodeEventCaption(void* item)
{
...
//---------------------------------------------------------------------------
TTreeNode* __fastcall TIdrDfmFormTree_11011981::FindTreeNodeByText(TTreeNode* nodeFrom, const AnsiString& txt, bool caseSensitive)
{
    TTreeNode* tn = 0;
    ...
    for (int i=0; i < nodes->Count; ++i)
    {
        ...
        AnsiString ttt = nodes->Item[i]->Text;
...
//---------------------------------------------------------------------------
void __fastcall TIdrDfmFormTree_11011981::tvFormKeyPress(TObject *Sender, TCHAR &Key)
{
    if (VK_RETURN == Key)
    {
        Key = 0;
