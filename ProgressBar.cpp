...
//---------------------------------------------------------------------------
void __fastcall TFProgressBar::StartProgress(AnsiString text0, AnsiString text1, int steps)
{
    pb->Min = 0;
    pb->Max = steps;
    pb->Step = 1;
...
void __fastcall TFProgressBar::wm_updAnalysisStatus(TMessage& msg)
{
    if (taStartPrBar == msg.WParam)
    {
        ...
        FProgressBar->StartProgress(startOperation ? startOperation->sbText : AnsiString(""), "", startOperation ? startOperation->pbSteps : 0);
    ...
    else if (taUpdateStBar == msg.WParam)
    {
        ...
        FProgressBar->sb->Panels->Items[1]->Text = updStatusBar ? updStatusBar->sbText : AnsiString("?");
