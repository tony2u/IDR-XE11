    ...
    while (1)
    {
        ...
        if (DisInfo.Ret)
        {
            ...
            if (!lastAdr || curAdr == lastAdr)
            {
                if (AnalyzeRetType)
                {
                    //If the EAX register type is not empty, find the topmost incident instruction
        ...
        //Check if the FixUp or ThreadVar instruction has entered
        ...
        if (b1 == 0xFF && (b2 & 0x38) == 0x20 && DisInfo.OpType[0] == otMEM && IsValidImageAdr(DisInfo.Offset)) //near absolute indirect jmp (Case)
        {
            ...
            //Analyze the gap based on the cTbl table
        ...
        if (registers[16].type != "" && registers[16].type[1] == '#')
        {
            ...
            //If there is a function call to @GetTls, please check the trajectory. View command [eax+N]
            if (dd == 'SLT#')
            {
        ...
        if (DisInfo.Call)
        {
            ...
            if (IsValidImageAdr(Adr))
            {
                ...
                if (recN && recN->procInfo)
                {
                    ...
                    {
                        //If dl=1, use EAX register after calling
...
//---------------------------------------------------------------------------
AnsiString __fastcall TFMain_11011981::AnalyzeTypes(DWORD parentAdr, int callPos, DWORD callAdr, PRINFO registers)
{
                                ...
                                else if (argInfo->Tag == 0x21)
                                {
                                    ...
                                    if (!recN1->HasName())
                                    {
                                        if (IsValidCodeAdr(itemAdr))
                                        {
											...
											LPWSTR wStr = (wchar_t*)(Code + itemPos);
