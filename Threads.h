...
{
    ThreadAnalysisData(int steps, const AnsiString& txt)
...
private:
    ...
    int __fastcall StartProgress(int pbMaxCount, const AnsiString& sbText);
    void __fastcall UpdateProgress();
    void __fastcall StopProgress();
    void __fastcall UpdateStatusBar(int adr);
    void __fastcall UpdateStatusBar(const AnsiString& sbText);
    ...
    void __fastcall FindVMTs2();                //For Delphi2 version (different structure!)
    void __fastcall FindVMTs();
    void __fastcall FindTypeFields();
    AnsiString __fastcall FindEvent(DWORD VmtAdr, AnsiString Name);
