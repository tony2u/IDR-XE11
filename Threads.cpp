...
int __fastcall TAnalyzeThread::StartProgress(int pbMaxCount, const AnsiString& sbText)
{
    int stepSize = 1;
...
//---------------------------------------------------------------------------
void __fastcall TAnalyzeThread::UpdateStatusBar(const AnsiString& sbText)
{
    //if (Terminated)
...
void __fastcall TAnalyzeThread::FindRTTIs()
{
    ...
    for (int i = 0; i < CodeSize && !Terminated; i += 4)
    {
        ...
        if (IsValidImageAdr(adr) && adr == Pos2Adr(i) + 4)
        {
            ...
            AnsiString TypeName = GetTypeName(adr);
            ...
            */
            ...
            case ikEnumeration:     //3
                ...
                {
                    unitName = AnsiString((LPTSTR)(Code + n + 1), len).Trim();
...
void __fastcall TAnalyzeThread::FindVMTs2()
{
    ...
    for (i = 0; i < CodeSize && !Terminated; i += 4)
    {
        ...
        if (_ap <= 0) continue;
        len = *(Code + _ap);
        TypeName = AnsiString((LPTSTR)(Code + _ap + 1), len);
        UpdateStatusBar(TypeName);
        ...
        if (recU)
        {
            if (typeInfoAdr)    //extract unit name
            {
                ...
                len = Code[_ap]; _ap++;
                if (!IsValidName(len, _ap)) continue;
                AnsiString unitName = AnsiString((LPTSTR)(Code + _ap), len).Trim(); _ap += len;
                FMain_11011981->SetUnitName(recU, unitName);
...
void __fastcall TAnalyzeThread::FindVMTs()
{
    ...
    for (int i = 0; i < TotalSize && !Terminated; i += 4)
    {
        ...
        if (IsValidImageAdr(adr) && Pos2Adr(i) == adr + cVmtSelfPtr)
        {
            ...
            {
                if (!IsValidImageAdr(typeInfoAdr)) continue;
                //TypeInfoadr addresses must be type data starting with specific information.
                pos = Adr2Pos(typeInfoAdr);
                BYTE typeKind = *(Code + pos);
            ...
            BYTE len = Code[n];
            //if (!IsValidName(len, n + 1)) continue;
            AnsiString TypeName = AnsiString((LPTSTR)(Code + n + 1), len);
            UpdateStatusBar(TypeName);
            ...
            if (recU)
            {
                ...
                if (adr && IsValidImageAdr(adr))
                {
                    ...
                    len = Code[pos]; pos++; bytes++;
                    if (!IsValidName(len, pos)) continue;
                    AnsiString unitName = AnsiString((LPTSTR)(Code + pos), len).Trim(); bytes += len;
                    FMain_11011981->SetUnitName(recU, unitName);
...
void __fastcall TAnalyzeThread::StrapVMTs()
{
    ...
    for (int i = 0; i < TotalSize && !Terminated; i += 4)
    {
        ...
        {
            AnsiString _name = recN->GetName();
            AnsiString ConstName = "_DV_" + _name;
    ...
    for (int i = 0; i < TotalSize && !Terminated; i += 4)
    {
    	...
    	if (recN && recN->kind == ikVMT)
        {
            ...
            if (recU)
            {
				...
				{
					LPTSTR text = TransformPChar(CP_ACP, recU->names->Strings[u].c_str(), -1);
					WORD ModuleID = KnowledgeBase.GetModuleID(text);
					delete[] text;
                        ...
                        if(KnowledgeBase.GetTypeInfo(TypeIdx, INFO_FIELDS, tInfo))
                        {
                            if (tInfo->Fields)
                            {
                                ...
                                for (int n = 0; n < tInfo->FieldsNum; n++)
                                {
                                    ...
                                    Len = *((WORD*)p); p += 2;
                                    fInfo.Type = TrimTypeName(AnsiString((LPTSTR)p, Len)); p += Len + 1;
...
//---------------------------------------------------------------------------
AnsiString __fastcall TAnalyzeThread::FindEvent(DWORD VmtAdr, AnsiString Name)
{
    DWORD adr = VmtAdr;
    while (adr)
    {
...
void __fastcall TAnalyzeThread::FindPrototypes()
{
    ...
    for (n = 0; n < ResInfo->FormList->Count && !Terminated; n++)
    {
        ...
        AnsiString className = dfm->ClassName;
        ...
        for (m = 0; m < ev->Count && !Terminated; m++)
        {
            ...
            AnsiString typeName = FindEvent(controlAdr, "F" + eInfo->EventName);
...
{
    bool    used;
    TCHAR    *name;   	//Unit Name
    float   matched;	//Maximum number of matches
    int     maxno;  	//Maximum number of matches
...
void __fastcall TAnalyzeThread::ScanCode()
{
    ...
    for (n = 0; n < UnitsNum && !Terminated; n++)
    {
        ...
        if (iniAdr && !recU->trivialIni)
        {
			...
			{
				LPTSTR text = TransformPChar(CP_ACP, recU->names->Strings[u].c_str(), -1);
				moduleID = KnowledgeBase.GetModuleID(text);
				delete[] text;
                if (moduleID == 0xFFFF) continue;

                recU->kb = true;
				//If unit is in knowledge base try to find proc Initialization
				text = TransformPChar(CP_ACP, recU->names->Strings[u].c_str(), -1);
				Idx = KnowledgeBase.GetProcIdx(moduleID, text);
				delete[] text;
                if (Idx != -1)
                {
                    Idx = KnowledgeBase.ProcOffsets[Idx].NamId;
    ...
    //Now, let's try using VMT again and search for them in the knowledge base.
    ...
    for (n = 0; n < TotalSize && !Terminated; n++)
    {
    	...
    	if (recN && recN->kind == ikVMT)
        {
            AnsiString ConstName = "_DV_" + recN->GetName();
                                ...
                                for (k = FirstProcIdx; k <= LastProcIdx && !Terminated; k++)
                                {
                                    ...
                                    if (!KnowledgeBase.IsUsedProc(Idx))
                                    {
                                        ...
                                        if (KnowledgeBase.GetProcInfo(Idx, INFO_DUMP | INFO_ARGS, pInfo))
                                        {
                                            ...
                                            if (matched)
                                            {
                                                AnsiString unitName = KnowledgeBase.GetModuleName(moduleID);
                                                FMain_11011981->SetUnitName(recU, unitName);
                                                recU->kb = true;
                                                mainForm->StrapProc(m, Idx, pInfo, true, pInfo->DumpSz);
                                                break;
    ...
    for (m = 0; m < UnitsNum && !Terminated; m++)
    {
        ...
        if (recU->names->Count)
        {
            for (u = 0; u < recU->names->Count && !Terminated; u++)
            {
				...
				LPTSTR text = TransformPChar(CP_ACP, recU->names->Strings[u].c_str(), -1);
				ModID = KnowledgeBase.GetModuleID(text);
				delete[] text;
                //If ResStr information is extracted from the knowledge base
                if (ModID != 0xFFFF)
                {
                    for (n = Adr2Pos(recU->fromAdr); (n < Adr2Pos(recU->toAdr)) && !Terminated; n += 4)
                    {
                        ...
                        if (IsValidImageAdr(adr) && adr == HInstanceVarAdr && resid < 0x10000)
                        {
                            ...
                            recN->rsInfo->value = AnsiString(buf, Bytes);
                            ...
                            else
                            {
                                if (!recN->HasName())
                                {
                                    recN->ConcatName("SResString" + AnsiString(LastResStrNo));
                                    LastResStrNo++;
                                }
                            }
                        }
    ...
    for (int m = 0; m < OwnTypeList->Count && !Terminated; m++)
    {
        ...
        if (recT->kind == ikClass)
        {
            ...
            for (int i = 0; i < propCount && !Terminated; i++)
            {
                ...
                posn++; //property type
                len = Code[posn]; posn++;
                AnsiString fieldType = AnsiString((LPTSTR)(Code + posn), len);
                ...
                n += 2;
                len = Code[n]; n++;
                AnsiString fieldName = AnsiString((LPTSTR)(Code + n), len); n += len;
...
{
    AnsiString      msg = "Analyzing step 1...";
    PUnitRec    recU;
...
void __fastcall TAnalyzeThread::PropagateClassProps()
{
    ...
    for (int n = 0; n < CodeSize && !Terminated; n += 4)
    {
    	...
    	if (recN && recN->HasName())
        {
            ...
            if (typeKind > ikProcedure) continue;
            //Viewing class properties and naming process
            if (typeKind == ikClass)
            {
                int pos = n;
                //SelfPointer
                ...
                AnsiString clsName = AnsiString((LPTSTR)(Code + pos), len); pos += len;
                ...
                for (int i = 0; i < propCount && !Terminated; i++)
                {
                    ...
                    posn++; //property type
                    len = Code[posn]; posn++;
                    AnsiString typeName = AnsiString((LPTSTR)(Code + posn), len);
...
void __fastcall TAnalyzeThread::AnalyzeDC()
{
    ...
    //Create a temporary pairing list (height, VMT address)
    const int cntVmt = VmtList->Count;
    if (!cntVmt) return;
