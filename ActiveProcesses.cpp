    ...
    if (_hSnapshot != INVALID_HANDLE_VALUE)
    {
        ...
        do
        {
            ...
            if (lpModule32First(_hSnapshotM, &_pme))
            {
                ...
                _li->SubItems->Add(ExtractFileName(AnsiString(_ppe.szExeFile)));
        ...
        if (ProcessIds[n])
        {
            ...
            if (_hProcess)
            {
				if (lpEnumProcessModules(_hProcess, ModuleHandles[0], sizeof(ModuleHandles), &ModulesNum))
                {
                    ...
                    _moduleName = AnsiString(_buf, _len);
...
void __fastcall TFActiveProcesses::DumpProcess(DWORD PID, TMemoryStream* MemStream, DWORD* BoC, DWORD* PoC, DWORD* ImB)
{
    ...
    if (_hProcess)
    {
        ...
        */
        //Correct PE Header
        //Set SectionNum = 2
		MemStream->Seek((__int64)(6 + _peHdrOffset), soFromBeginning);
        _secNum = 2;
        MemStream->WriteBuffer(&_secNum, sizeof(_secNum));
        //Set EP
        //Set sections
        MemStream->Seek((__int64)(0xF8 + _peHdrOffset), soFromBeginning);
        //"CODE"
