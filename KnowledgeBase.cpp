...
//---------------------------------------------------------------------------
#define cfCode          0x00000001  //Byte refers to code
#define cfData          0x00000002  //Bytes refer to data
FIELDINFO::~FIELDINFO()
{
...
//---------------------------------------------------------------------------
bool __fastcall MKnowledgeBase::Open(LPTSTR filename)
{
    if (Inited)
    {
        if (NameKBFile == AnsiString(filename))
            return true;
        Close();
    }
    ...
    NameKBFile = AnsiString(filename);
    Inited = true;
    return true;
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
WORD __fastcall MKnowledgeBase::GetModuleID(LPTSTR ModuleName)
{
...
//---------------------------------------------------------------------------
AnsiString __fastcall MKnowledgeBase::GetModuleName(WORD ModuleID)
{
	if (!Inited) return "";
    ...
    p = GetKBCachePtr(ModuleOffsets[ID].Offset, ModuleOffsets[ID].Size);
    ModID = *((WORD*)p);
    if (ModuleID == ModID) return AnsiString((LPTSTR)(p + 4));
    return "";
}
//---------------------------------------------------------------------------
//Return modules ids list containing given proc
void __fastcall MKnowledgeBase::GetModuleIdsByProcName(LPTSTR AProcName)
{
    Mods[0] = 0xFFFF;
...
int __fastcall MKnowledgeBase::GetItemSection(WORD* ModuleIDs, LPTSTR ItemName)
{
    bool    found;
...
int __fastcall MKnowledgeBase::GetConstIdx(WORD* ModuleIDs, LPTSTR ConstName)
{
	if (!Inited) return -1;
...
//---------------------------------------------------------------------------
int __fastcall MKnowledgeBase::GetConstIdxs(LPTSTR ConstName, int* ConstIdx)
{
    *ConstIdx = -1;
...
//---------------------------------------------------------------------------
int __fastcall MKnowledgeBase::GetTypeIdxByModuleIds(WORD* ModuleIDs, LPTSTR TypeName)
{
	if (!Inited) return -1;
...
//---------------------------------------------------------------------------
int __fastcall MKnowledgeBase::GetTypeIdxsByName(LPTSTR TypeName, int* TypeIdx)
{
    *TypeIdx = -1;
...
//---------------------------------------------------------------------------
int __fastcall MKnowledgeBase::GetTypeIdxByUID(LPTSTR UID)
{
}
//---------------------------------------------------------------------------
int __fastcall MKnowledgeBase::GetVarIdx(WORD* ModuleIDs, LPTSTR VarName)
{
	if (!Inited) return -1;
...
//---------------------------------------------------------------------------
int __fastcall MKnowledgeBase::GetResStrIdx(int from, LPTSTR ResStrContext)
{
	if (!Inited) return -1;
...
//---------------------------------------------------------------------------
int __fastcall MKnowledgeBase::GetResStrIdx(WORD ModuleID, LPTSTR ResStrContext)
{
	if (!Inited) return -1;
...
//---------------------------------------------------------------------------
int __fastcall MKnowledgeBase::GetResStrIdx(WORD* ModuleIDs, LPTSTR ResStrName)
{
	if (!Inited) return -1;
...
int __fastcall MKnowledgeBase::GetProcIdx(WORD ModuleID, LPTSTR ProcName)
{
	if (!Inited) return -1;
...
int __fastcall MKnowledgeBase::GetProcIdx(WORD ModuleID, LPTSTR ProcName, BYTE* code)
{
	if (!Inited) return -1;
...
int __fastcall MKnowledgeBase::GetProcIdx(WORD* ModuleIDs, LPTSTR ProcName, BYTE* code)
{
	if (!Inited) return -1;
...
MConstInfo* __fastcall MKnowledgeBase::GetConstInfo(int AConstIdx, DWORD AFlags, MConstInfo* cInfo)
{
    ...
    WORD Len = *((WORD*)p); p += 2;
    cInfo->ConstName = AnsiString((LPTSTR)p); p += Len + 1;
    cInfo->Type = *p; p++;
    Len = *((WORD*)p); p += 2;
    cInfo->TypeDef = AnsiString((LPTSTR)p); p += Len + 1;
    Len = *((WORD*)p); p += 2;
    cInfo->Value = AnsiString((LPTSTR)p); p += Len + 1;
...
//---------------------------------------------------------------------------
MProcInfo* __fastcall MKnowledgeBase::GetProcInfo(LPTSTR ProcName, DWORD AFlags, MProcInfo *pInfo, int *procIdx)
{
	if (!Inited) return 0;
        ...
        else
        {
            ...
            if (RN - LN - 1 == 1)
            {
                ...
                *procIdx = ID;
                ...
                WORD Len = *((WORD*)p); p += 2;
                pInfo->ProcName = AnsiString((LPTSTR)p); p += Len + 1;
                pInfo->Embedded = *p; p++;
                pInfo->DumpType = *p; p++;
                ...
                Len = *((WORD*)p); p += 2;
                pInfo->TypeDef = TrimTypeName(AnsiString((LPTSTR)p)); p += Len + 1;
...
MTypeInfo* __fastcall MKnowledgeBase::GetTypeInfo(int ATypeIdx, DWORD AFlags, MTypeInfo *tInfo)
{
    ...
    WORD Len = *((WORD*)p); p += 2;
    tInfo->TypeName = AnsiString((LPTSTR)p); p += Len + 1;
    tInfo->Kind = *p; p++;
    tInfo->VMCnt = *((WORD*)p); p += 2;
    Len = *((WORD*)p); p += 2;
    tInfo->Decl = AnsiString((LPTSTR)p); p += Len + 1;
...
MVarInfo* __fastcall MKnowledgeBase::GetVarInfo(int AVarIdx, DWORD AFlags, MVarInfo* vInfo)
{
    ...
    WORD Len = *((WORD*)p); p += 2;
    vInfo->VarName = AnsiString((LPTSTR)p); p += Len + 1;
    vInfo->Type = *p; p++;
    Len = *((WORD*)p); p += 2;
    vInfo->TypeDef = AnsiString((LPTSTR)p); p += Len + 1;

    if (AFlags & INFO_ABSNAME)
    {
        Len = *((WORD*)p); p += 2;
        vInfo->AbsName = AnsiString((LPTSTR)p);
    }
...
MResStrInfo* __fastcall MKnowledgeBase::GetResStrInfo(int AResStrIdx, DWORD AFlags, MResStrInfo* rsInfo)
{
    ...
    WORD Len = *((WORD*)p); p += 2;
    rsInfo->ResStrName = AnsiString((LPTSTR)p); p += Len + 1;
    Len = *((WORD*)p); p += 2;
    rsInfo->TypeDef = AnsiString((LPTSTR)p); p += Len + 1;

    if (AFlags & INFO_DUMP)
    {
        //Len = *((WORD*)p); p += 2;
        //rsInfo->AContext = AnsiString((LPTSTR)p);
    }
...
//---------------------------------------------------------------------------
int __fastcall MKnowledgeBase::GetProcUses(LPTSTR ProcName, WORD* uses)
{
	if (!Inited) return 0;
...
//---------------------------------------------------------------------------
WORD* __fastcall MKnowledgeBase::GetTypeUses(LPTSTR TypeName)
{
	if (!Inited) return 0;
...
//---------------------------------------------------------------------------
WORD* __fastcall MKnowledgeBase::GetConstUses(LPTSTR ConstName)
{
	if (!Inited) return 0;
...
//---------------------------------------------------------------------------
AnsiString __fastcall MKnowledgeBase::GetProcPrototype(int ProcIdx)
{
    MProcInfo pInfo;
...
//---------------------------------------------------------------------------
AnsiString __fastcall MKnowledgeBase::GetProcPrototype(MProcInfo* pInfo)
{
    AnsiString Result = "";
    ...
    if (pInfo)
    {
        ...
        for (int n = 0; n < pInfo->ArgsNum; n++)
        {
            ...
            //Name
            Len = *((WORD*)p); p += 2;
            Result += AnsiString((LPTSTR)p, Len) + ":"; p += Len + 1;
            //TypeDef
            Len = *((WORD*)p); p += 2;
            Result += AnsiString((LPTSTR)p, Len); p += Len + 1;
        }
...
//---------------------------------------------------------------------------
bool __fastcall MKnowledgeBase::GetKBProcInfo(AnsiString typeName, MProcInfo* procInfo, int* procIdx)
{
    int     res, idx;
...
//---------------------------------------------------------------------------
bool __fastcall MKnowledgeBase::GetKBTypeInfo(AnsiString typeName, MTypeInfo* typeInfo)
{
    int         idx;
    WORD        *uses;
...
//---------------------------------------------------------------------------
bool __fastcall MKnowledgeBase::GetKBPropertyInfo(AnsiString className, AnsiString propName, MTypeInfo* typeInfo)
{
    int         n, idx, pos;
    BYTE        *p;
    WORD        *uses, Len;
    MTypeInfo   tInfo;
    AnsiString      name, type;
    ...
    if (idx != -1)
    {
        ...
        if (GetTypeInfo(idx, INFO_PROPS, &tInfo))
        {
            ...
            for (n = 0; n < tInfo.PropsNum; n++)
            {
                ...
                p += 4;//DispID
                Len = *((WORD*)p); p += 2;
                name = AnsiString((LPTSTR)p, Len); p += Len + 1;//Name
                Len = *((WORD*)p); p += 2;
                type = TrimTypeName(AnsiString((LPTSTR)p, Len)); p += Len + 1;//TypeDef
...
//---------------------------------------------------------------------------
AnsiString __fastcall MKnowledgeBase::IsPropFunction(AnsiString className, AnsiString procName)
{
    int         n, idx;
    BYTE        *p;
    WORD        *uses, Len;
    MTypeInfo   tInfo;
    AnsiString      pname, type, fname;
    ...
    if (idx != -1)
    {
        ...
        if (GetTypeInfo(idx, INFO_PROPS, &tInfo))
        {
            ...
            for (n = 0; n < tInfo.PropsNum; n++)
            {
                ...
                p += 4;//DispID
                Len = *((WORD*)p); p += 2;
                pname = AnsiString((LPTSTR)p, Len); p += Len + 1;//Name
                Len = *((WORD*)p); p += 2;
                type = TrimTypeName(AnsiString((LPTSTR)p, Len)); p += Len + 1;//TypeDef
                Len = *((WORD*)p); p += 2;
                fname = TrimTypeName(AnsiString((LPTSTR)p, Len)); p += Len + 1;//ReadName
                if (SameText(procName, fname))
                    return pname;
                Len = *((WORD*)p); p += 2;
                fname = TrimTypeName(AnsiString((LPTSTR)p, Len)); p += Len + 1;//WriteName
                if (SameText(procName, fname))
                    return pname;
                Len = *((WORD*)p); p += 2;
                fname = TrimTypeName(AnsiString((LPTSTR)p, Len)); p += Len + 1;//StoredName
                if (SameText(procName, fname))
                    return pname;
            }
        }
