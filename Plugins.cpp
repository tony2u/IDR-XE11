...
void __fastcall TFPlugins::FormShow(TObject *Sender)
{
    ...
    if (PluginsPath != "")
    {
        ...
        if (!FindFirst("*.dll", faArchive, sr))
        {
            do
            {
                HINSTANCE hModule = LoadLibraryW(sr.Name.c_str());
                if (hModule)
                {
                    AnsiString info = "";
...
{
    AnsiString filename = "";
    AnsiString line = cklbPluginsList->Items->Strings[cklbPluginsList->ItemIndex];
    int pos = line.Pos('-');
...
void __fastcall TFPlugins::bOkClick(TObject *Sender)
{
    ...
    for (int n = 0; n < cklbPluginsList->Items->Count; n++)
    {
        ...
        {
            AnsiString line = cklbPluginsList->Items->Strings[n];
            int pos = line.Pos('-');
