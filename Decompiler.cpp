...
extern  TCHAR        StringBuf[MAXSTRBUFFER];
extern  MKnowledgeBase  KnowledgeBase;
//---------------------------------------------------------------------------
AnsiString __fastcall GetString(PITEM item, BYTE precedence)
{
    AnsiString  _val = item->Value;
...
//---------------------------------------------------------------------------
AnsiString __fastcall GetFieldName(PFIELDINFO fInfo)
{
    if (fInfo->Name == "") return AnsiString("?f") + Val2Str0(fInfo->Offset);
    return fInfo->Name;
}
//---------------------------------------------------------------------------
AnsiString __fastcall GetArgName(PARGINFO argInfo)
{
    if (argInfo->Name == "") return AnsiString("arg_") + Val2Str0(argInfo->Ndx);
    return argInfo->Name;
}
//---------------------------------------------------------------------------
AnsiString __fastcall GetGvarName(DWORD adr)
{
...
//'Q'-in;'R'-not in;'S'-is
AnsiString DirectConditions[19] = {"", "", "<", ">=", "=", "<>", "<=", ">", "", "", "", "", "<", ">=", "<=", ">", "not in", "in", "is"};
AnsiString __fastcall GetDirectCondition(TCHAR c)
{
    if (c >= 'A')
...
//---------------------------------------------------------------------------
AnsiString InvertConditions[19] = {"", "", ">=", "<", "<>", "=", ">", "<=", "", "", "", "", ">=", "<", ">", "<=", "in", "not in", "is not"};
AnsiString __fastcall GetInvertCondition(TCHAR c)
{
    if (c >= 'A')
...
//---------------------------------------------------------------------------
AnsiString __fastcall TNamer::MakeName(AnsiString shablon)
{
    TCHAR    name[1024];
    ...
    if (shablon[1] == 'T')
    {
        ...
        for (int n = 0; n <= MaxIdx; n++)
        {
            ...
            if (Names->IndexOf(AnsiString(name)) != -1) idx = n;
        }
        if (idx == -1)
        {
            Names->Add(AnsiString(name));
...
//---------------------------------------------------------------------------
__fastcall TForInfo::TForInfo(bool ANoVar, bool ADown, int AStopAdr, AnsiString AFrom, AnsiString ATo, BYTE AVarType, int AVarIdx, BYTE ACntType, int ACntIdx)
{
    NoVar = ANoVar;
    Down = ADown;
    StopAdr = AStopAdr;
...
//---------------------------------------------------------------------------
AnsiString __fastcall TDecompileEnv::GetLvarName(int Ofs, AnsiString Type)
{
    PLOCALINFO  locInfo;
    AnsiString  _defaultName = AnsiString("lvar_") + Val2Str0(LocBase - Ofs);
...
//---------------------------------------------------------------------------
AnsiString __fastcall TDecompiler::GetRegType(int Idx)
{
...
//---------------------------------------------------------------------------
void __fastcall TDecompileEnv::OutputSourceCodeLine(AnsiString line)
{
    int spaceNum = TAB_SIZE * Indent;
    memset(StringBuf, ' ', spaceNum);
    int len = sprintf(StringBuf + spaceNum, "%s", line.c_str());

    FMain_11011981->lbSourceCode->Items->Add(AnsiString(StringBuf));
}
//---------------------------------------------------------------------------
void __fastcall TDecompileEnv::OutputSourceCode()
{
    bool    _end;
    AnsiString  line, nextline;
    ...
    for (int n = 0; n < Body->Count; n++)
    {
        ...
        {
            if (SameText(line, "begin")) line += "//" + AnsiString(Indent);
        ...
        if (SameText(line, "end") || SameText(line, "until"))
        {
            ...
            if (_end) line += "//" + AnsiString(Indent);
            OutputSourceCodeLine(line);
            continue;
        }
        OutputSourceCodeLine(line);
...
void __fastcall TDecompileEnv::DecompileProc()
{
    ...
    if (_recN->procInfo->locals && _recN->procInfo->locals->Count > 0)
    {
        ...
        {
            PLOCALINFO locInfo = PLOCALINFO(_recN->procInfo->locals->Items[n]);
            AnsiString line = "  " + GetLvarName(locInfo->Ofs, locInfo->TypeDef) + ":" + locInfo->TypeDef + ";";
            AddToBody(line);
        }
    }
    ///////////////////////////
...
void __fastcall TDecompileEnv::CreateBJLSequence(DWORD fromAdr, DWORD bodyBegAdr, DWORD bodyEndAdr)
{
    ...
    for (i = 0, m = 1, BJLmaxbcnt = 0; i < bjllist->Count; i++)
    {
        ...
        if (_bjl1->type == BJL_BRANCH)
        {
            ...
            _bjlInfo->dExpr = "#" + AnsiString(m) + "#" + _bjlInfo->dExpr + "#" + AnsiString(m + 1) + "#";
            _bjlInfo->iExpr = "#" + AnsiString(m) + "#" + _bjlInfo->iExpr + "#" + AnsiString(m + 1) + "#";
            m += 2;
...
//---------------------------------------------------------------------------
bool __fastcall TDecompileEnv::BJLCheckPattern1(LPTSTR t, int from)
{
    TBJL    *_bjl;
...
//---------------------------------------------------------------------------
bool __fastcall TDecompileEnv::BJLCheckPattern2(LPTSTR t, int from)
{
    int     _address = -1;
    TBJL    *_bjl;
...
//---------------------------------------------------------------------------
TCHAR __fastcall TDecompileEnv::ExprGetOperation(AnsiString s)
{
    TCHAR    c;
    int     n = 0;
...
//---------------------------------------------------------------------------
void __fastcall TDecompileEnv::ExprMerge(AnsiString& dst, AnsiString src, TCHAR op)//dst = dst op src, op = '|' or '&'
{
    TCHAR    op1, op2;
...
//---------------------------------------------------------------------------
AnsiString __fastcall TDecompileEnv::PrintBJL()
{
    int         n, m, k;
    AnsiString      _result = "";
    CMPITEM     *_cmpItem;
    ...
    for (n = 0; n < BJLseq->Count; n++)
    {
        ...
        if (_bjlInfo->state == 'I')
        {
            ...
            for (m = 0, k = 1; m < CmpStack->Count; m++)
            {
                ...
                _result = AnsiReplaceText(_result, "#" + AnsiString(k) + "#", "(" + _cmpItem->L + " "); k++;
                _result = AnsiReplaceText(_result, "#" + AnsiString(k) + "#", " " + _cmpItem->R + ")"); k++;
            }
            break;
        }
    }
...
DWORD __fastcall TDecompiler::Decompile(DWORD fromAdr, DWORD flags, PLoopInfo loopInfo)
{
    ...
    AnsiString          _line, _comment, _name, _typeName;
    DISINFO         _disInfo;
    TDecompiler     *de;
...
_curAdr = _curAdr;
        ...
        if (_op == OP_TEST || _op == OP_BT)
        {
            ...
            if (_bytesToSkip)
            {
                ...
                _item.Value = GetString(&_item, PRECEDENCE_MULT) + " Div " + AnsiString(_div);
                _item.Type = "Integer";
                SetRegItem(DisInfo.OpRegIdx[0], &_item);
        ...
        if (_op == OP_AND)
        {
            ...
            if (_bytesToSkip)
            {
                GetRegItem(DisInfo.OpRegIdx[0], &_item);
                _item.Precedence = PRECEDENCE_ATOM;
                _item.Value = _item.Value + " mod " + AnsiString(_mod);
                _item.Type = "Integer";
                SetRegItem(DisInfo.OpRegIdx[0], &_item);

                _line = GetDecompilerRegisterName(DisInfo.OpRegIdx[0]) + " := " + GetDecompilerRegisterName(DisInfo.OpRegIdx[0]) + " mod " + AnsiString(_mod);
                Env->AddToBody(_line);
...
bool __fastcall TDecompiler::SimulateCall(DWORD curAdr, DWORD callAdr, int instrLen, PMethodRec recM, DWORD AClassAdr)
{
    ...
    MTypeInfo   _tInfo;
    TDecompiler *de;
    AnsiString      _name, _type, _alias, _line, _retType, _value, _iname, _embAdr;
    AnsiString      _typeName, _comment, _regName, _propName, _lvarName;
        ...
        if (!recM)
        {
            ...
            if (_recN->procInfo->flags & PF_EMBED)
            {
                ...
                if (Env->EmbeddedList->IndexOf(_embAdr) == -1)
                {
                    ...
                    if (Application->MessageBox(String("Decompile embedded procedure at address " + _embAdr + "?").c_str(), (LPWSTR)"Confirmation", MB_YESNO) == IDYES)
                    {
        ...
        if (_argsNum)
        {
            ...
            if (_callKind == 0 || _callKind == 3 || _callKind == 2 || _callKind == 1)//fastcall, stdcall, pascal, cdecl
            {
                ...
                for (int n = 0; n < _argsNum; n++)
                {
                    ...
                    {
                        _line += _item.Value;
                        if (_item.Flags & IF_INTVAL) _line += "{'" + AnsiString(_item.IntValue) + "'}";
                        continue;
                    }
                            ...
                            if (_item.Flags & IF_INTVAL)
                            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        if (_kind == ikCString)
                                        {
                                            _line += "'" + AnsiString((LPTSTR)(Code + Adr2Pos(_item.IntValue))) + "'";
                                        }
                    ...
                    if (_item.Value != "")
                        ...
                        _line += AnsiString(_item.IntValue);
                    else
                        _line += "?";
                    continue;
                }
    ...
    if (DisInfo.BaseReg != -1)
    {
        ...
        if (IsValidImageAdr(_classAdr))
        {
            ...
            {
                Env->AddToBody(_item.Value + ".I" + AnsiString(DisInfo.Offset) + "(...);");
                _value = ManualInput(CurProcAdr, curAdr, "Input return bytes number (in hex) of procedure at " + Val2Str8(curAdr), "Bytes:");
                if (_value == "")
                {
                    Env->ErrAdr = curAdr;
    ...
    if (DisInfo.OpNum == 1 && DisInfo.OpType[0] == otMEM)
    {
        ...
        while (_argsNum)
        {
            ...
            if (_item.Flags & IF_INTVAL)
                Env->AddToBody(AnsiString(_item.IntValue));
            else
    ...
    if (recN && recN->xrefs)
    {
                        ...
                        if (_disInfo.BaseReg == 21)//[ebp-N]
                        {
                            ...
                            if (item.IntValue + _disInfo.Offset == varIdxInfo.IdxValue)
                            {
                                if (Env->Stack[varIdxInfo.IdxValue].Flags & IF_INTVAL)
                                    from = AnsiString(Env->Stack[varIdxInfo.IdxValue].IntValue);
                                else
        ...
        if (_dd1 == 'cni' || _dd1 == 'ced')
        {
            ...
            while (1)
            {
                pos = GetNearestUpInstruction(pos);
                instrLen = Disasm.Disassemble(Code + pos, (__int64)Pos2Adr(pos), &_disInfo, 0);
                if (SameText(cntIdxInfo.IdxStr, AnsiString(_disInfo.Op1)))
                    break;
            }
            //3
            pos += instrLen;
            ...
            else
            {
                if (varIdxInfo.IdxType == itREG)
                {
                    ...
                    if (item.Flags & IF_INTVAL)
                    {
                        if (from == "") from = AnsiString(item.IntValue);
                        item.Flags &= ~IF_INTVAL;
                    }
                    else
                ...
                if (cntIdxInfo.IdxType == itREG)
                {
                    ...
                    if (item.Flags & IF_INTVAL)
                    {
                        if (cnt == "") cnt = AnsiString(item.IntValue);
                        item.Flags &= ~IF_INTVAL;
                    }
                    else
...
void __fastcall TDecompiler::SimulatePush(DWORD curAdr, bool bShowComment)
{
    ...
    ITEM        _item, _item1;
    PInfoRec    _recN;
    AnsiString      _name, _type, _typeName, _value, _regname;
...
void __fastcall TDecompiler::SimulateInstr1(DWORD curAdr, BYTE Op)
{
    ...
    AnsiString      _name, _value, _line, _comment;
...
void __fastcall TDecompiler::SimulateInstr2RegImm(DWORD curAdr, BYTE Op)
{
    ...
    PInfoRec    _recN, _recN1;
    PLOCALINFO  _locInfo;
	AnsiString  _name, _type, _value, _typeName, _line, _comment, _imm, _iname, _fname, _text;
	//WideString  _wStr;
	LPWSTR      _wStr;
    ...
    if (Op == OP_SAL || Op == OP_SHL)
    {
        ...
        if (Op == OP_SHL)
        {
            _item.Value = GetString(&_item1, PRECEDENCE_MULT) + " Shl " + AnsiString(DisInfo.Immediate);
            _line = GetDecompilerRegisterName(_reg1Idx) + " := " + GetDecompilerRegisterName(_reg1Idx) + " Shl " + AnsiString(DisInfo.Immediate);
            _comment = GetString(&_item1, PRECEDENCE_MULT) + " * " + AnsiString(_pow2);
        }
        else
        {
            _item.Value = GetString(&_item1, PRECEDENCE_MULT) + " * " + AnsiString(_pow2);
            _line = GetDecompilerRegisterName(_reg1Idx) + " := " + GetDecompilerRegisterName(_reg1Idx) + " * " + AnsiString(_pow2);
            _comment = "";
        }
        SetRegItem(_reg1Idx, &_item);
    ...
    if (Op == OP_SAR || Op == OP_SHR)
    {
        ...
        if (Op == OP_SHR)
        {
            _item.Value = GetString(&_item1, PRECEDENCE_MULT) + " Shr " + AnsiString(DisInfo.Immediate);
            _line = GetDecompilerRegisterName(_reg1Idx) + " := " + GetDecompilerRegisterName(_reg1Idx) + " Shr " + AnsiString(DisInfo.Immediate);
            _comment = GetString(&_item1, PRECEDENCE_MULT) + " Div " + AnsiString(_pow2);
        }
        else
        {
            _item.Value = GetString(&_item1, PRECEDENCE_MULT) + " Div " + AnsiString(_pow2);
            _line = GetDecompilerRegisterName(_reg1Idx) + " := " + GetDecompilerRegisterName(_reg1Idx) + " Div " + AnsiString(_pow2);
            _comment = "";
        }
        SetRegItem(_reg1Idx, &_item);
        if (_comment != "") _line += ";//" + _comment;
...
void __fastcall TDecompiler::SimulateInstr2RegReg(DWORD curAdr, BYTE Op)
{
    ...
    PLOCALINFO  _locInfo;
    PFIELDINFO  _fInfo;
    AnsiString      _name, _value, _typeName, _line, _comment, _imm, _iname, _fname, _op;
...
void __fastcall TDecompiler::SimulateInstr2RegMem(DWORD curAdr, BYTE Op)
{
    ...
    PLOCALINFO  _locInfo;
    PFIELDINFO  _fInfo;
    AnsiString      _name, _value, _type, _line, _comment, _imm, _iname, _fname, _op;
...
void __fastcall TDecompiler::SimulateInstr2MemImm(DWORD curAdr, BYTE Op)
{
    ...
    PLOCALINFO  _locInfo;
    PFIELDINFO  _fInfo;
    AnsiString      _name, _value, _typeName, _line, _comment, _imm, _iname, _fname, _key, _type, _op;
...
void __fastcall TDecompiler::SimulateInstr2MemReg(DWORD curAdr, BYTE Op)
{
    ...
    PLOCALINFO  _locInfo;
    PFIELDINFO  _fInfo;
    AnsiString      _name, _value, _typeName, _line, _comment, _imm, _iname, _fname, _key;
...
void __fastcall TDecompiler::SimulateInstr3(DWORD curAdr, BYTE Op)
{
    ...
    AnsiString      _value, _line, _comment;
    ...
    if (Op == OP_IMUL)
    {
        ...
        if (DisInfo.OpType[1] == otREG)
        {
            _value = AnsiString(_imm) + " * " + GetDecompilerRegisterName(_reg2Idx);
            _line = GetDecompilerRegisterName(_reg1Idx) + " := " + _value;
            GetRegItem(_reg2Idx, &_itemSrc);
            _comment = AnsiString(_imm) +  " * " + GetString(&_itemSrc, PRECEDENCE_MULT);
            _itemDst.Precedence = PRECEDENCE_MULT;
        ...
        {
            GetMemItem(curAdr, &_itemSrc, Op);
            _value = AnsiString(_imm) + " * " + _itemSrc.Name;
           _line = GetDecompilerRegisterName(_reg1Idx) + " := " + _value;
            _comment = AnsiString(_imm) +  " * " + GetString(&_itemSrc, PRECEDENCE_MULT);
            _itemDst.Precedence = PRECEDENCE_MULT;
            _itemDst.Value = _comment;
...
//---------------------------------------------------------------------------
AnsiString __fastcall TDecompiler::GetSysCallAlias(AnsiString AName)
{
...
//---------------------------------------------------------------------------
bool __fastcall TDecompiler::SimulateSysCall(AnsiString name, DWORD procAdr, int instrLen)
{
    int         _cnt, _esp, _cmpRes, _size;
    DWORD       _adr;
    ITEM        _item, _item1, _item2, _item3, _item4;
    PInfoRec    _recN;
    AnsiString      _line, _value, _value1, _value2, _typeName, _varName1, _varName2, _op;
    __int64     _int64Val;
    ...
    if (SameText(name, "@GetMem"))
    {
        ...
        GetRegItem(16, &_item);
        if (_item.Flags & IF_INTVAL)
            _value += AnsiString(_item.IntValue);
        else
            _value += _item.Value;
        _value += ")";
        _line = _value + ";";
        ...
        SameText(name, "@FinalizeArray"))
    {
        ...
        //ecx - dims cnt
        GetRegItem(17, &_item3); _cnt = _item3.IntValue;
        _typeName = "array [1.." + AnsiString(_cnt) + "] of " + GetTypeName(_item2.IntValue);
        if (_item1.Flags & IF_STACK_PTR)
        {
        ...
        SameText(name, "@UStrCatN"))
    {
        ...
        if (_item.Flags & IF_STACK_PTR)
        {
            ...
            if (name[2] == 'L')
                Env->Stack[_item.IntValue].Type = "String";
            else if (name[2] == 'W')
...
//Fix it!!!
//Add analyze of AnsiString type
    ...
    if (SameText(name, "@Str1Ext"))
    {
        ...
        _line = "Str(" + _item1.Value + ":" + AnsiString(_item2.IntValue) + ", " + _item3.Value + ");";
        Env->AddToBody(_line);
        return false;
    }
    if (SameText(name, "@Str2Ext"))
    {
        ...
        _line = "Str(" + _item1.Value + ":" + AnsiString(_item2.IntValue) + ":" + AnsiString(_item3.IntValue) + ", " + _item4.Value + ");";
        Env->AddToBody(_line);
        return false;
    }
    if (SameText(name, "@Write2Ext"))
    {
        ...
        //ecx
        GetRegItem(17, &_item4);
        _line = "Write(" + ExtractClassName(_item1.Value) + ", " + _item2.Value + ":" + AnsiString(_item3.IntValue) + ":" + AnsiString(_item4.IntValue) + ");";
        Env->AddToBody(_line);
        return false;
    }
...
void __fastcall TDecompiler::SimulateFloatInstruction(DWORD curAdr)
{
    ...
    PInfoRec    _recN;
    PFIELDINFO  _fInfo;
    AnsiString      _name, _typeName, _val, _line, _lvarName;
...
//---------------------------------------------------------------------------
void __fastcall TDecompileEnv::AddToBody(AnsiString src)
{
    Body->Add(src);
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
AnsiString __fastcall TDecompiler::GetCycleFrom()
{
    ITEM        _item;

    if (DisInfo.OpType[1] == otIMM) return AnsiString(DisInfo.Immediate);
    if (DisInfo.OpType[1] == otREG)
    {
...
//---------------------------------------------------------------------------
AnsiString __fastcall TDecompiler::GetCycleTo()
{
    ITEM        _item;
...
DWORD __fastcall TDecompiler::DecompileGeneralCase(DWORD fromAdr, DWORD markAdr, PLoopInfo loopInfo, int N)
{
    ...
    while (1)
    {
        ...
        if ((_dd == 'bus' && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otIMM) || (_dd == 'ced' && _disInfo.OpType[0] == otREG))
        {
            ...
            if (_dd == 'bj')
            {
                Env->AddToBody(AnsiString(_N1 - _N2) + ".." + AnsiString(_N1 - 1) + ":");
                _begAdr = _disInfo.Immediate;
                Env->SaveContext(_begAdr);
                de = new TDecompiler(Env);
                de->SetStackPointers(this);
...
//---------------------------------------------------------------------------
int __fastcall TDecompiler::GetArrayFieldOffset(AnsiString ATypeName, int AFromOfs, int AScale, AnsiString& name, AnsiString& type)
{
    bool        _vmt;
...
void __fastcall TDecompiler::GetFloatItemFromStack(int Esp, PITEM Dst, int FloatType)
{
    ...
    if (FloatType == FT_CURRENCY)
    {
		...
		String wstr = _currVal.operator String();
		LPTSTR str = TransformPChar(CP_ACP, wstr.c_str(), -1);
		Dst->Value = str;
		Dst->Type = "Currency";
		delete[] str;
        return;
...
void __fastcall TDecompiler::GetMemItem(int CurAdr, PITEM Dst, BYTE Op)
{
    ...
    AnsiString      _fname, _name, _type, _typeName, _iname, _value, _text, _lvarName;
    ...
    if (DisInfo.IndxReg == -1)
    {
        ...
        if (_kind == ikLString || _kind == ikString)
        {
            _value = _iname + "[" + AnsiString(_offset + 1) + "]";
            _typeName = "Char";
        }
        ...
        {
            _value = _iname + "[ofs=" + AnsiString(_offset) + "]";
        }
        else if (_kind == ikDynArray)
        {
            ...
            if (GetTypeKind(_typeName, &_size) == ikRecord)
            {
                ...
                _value = _itemBase.Value + "[" + AnsiString(_k) + "]";
                if (_text.Pos(":"))
                {
                    _value += "." + ExtractName(_text);
                    _typeName = ExtractType(_text);
        ...
        else if (_kind == ikVMT || _kind == ikClass)
        {
            ...
            //{
                ...
                if (_kind == ikArray)
                {
                    _value += "[ofs=" + AnsiString(_offset - _foffset) + "]";
                }
    ...
    else
    {
        ...
        if (Op == OP_LEA)
        {
            ...
            Dst->Value = GetDecompilerRegisterName(DisInfo.BaseReg) + " + " + GetDecompilerRegisterName(DisInfo.IndxReg) + " * " + AnsiString(DisInfo.Scale);
            if (_offset > 0)
                Dst->Value += " + " + AnsiString(_offset);
            else if (_offset < 0)
                Dst->Value += " - " + AnsiString(-_offset);
...
//---------------------------------------------------------------------------
AnsiString __fastcall TDecompiler::GetStringArgument(PITEM item)
{
...
//@LStrFromChar=Chr
//@LStrToPChar=PChar
//@LStrFromArray=AnsiString(src,len)
//---------------------------------------------------------------------------
//Constructions
//---------------------------------------------------------------------------
//Length
