...
public:
    ...
    __fastcall ~MDisasm();
    int __fastcall Init();
    int __fastcall Disassemble(DWORD fromAdr, PDISINFO pDisInfo, LPTSTR disLine);
    int __fastcall Disassemble(BYTE* from, __int64 address, PDISINFO pDisInfo, LPTSTR disLine);
    int __fastcall GetRegister(LPTSTR reg);
    HINSTANCE   hModule;
    BYTE __fastcall GetOp(LPTSTR mnem);
    ...
    LPTSTR __fastcall GetSizeString(int size);
private:
    ...
    int __fastcall GetOpType(LPTSTR Op);
    void __fastcall FormatInstr(PDISINFO pDisInfo, LPTSTR disLine);
    void __fastcall FormatArg(int argno, DWORD cmd, DWORD arg, PDISINFO pDisInfo, LPTSTR disLine);
    int __fastcall OutputGeneralRegister(TCHAR *dst, int reg, int size);
    void __fastcall OutputHex(TCHAR *dst, DWORD val);
    DWORD __fastcall GetAddress();
    void __fastcall OutputSegPrefix(LPTSTR dst, PDISINFO pDisInfo);
    int __fastcall EvaluateOperandSize();
    void __fastcall OutputSizePtr(int size, bool mm, PDISINFO pDisInfo, LPTSTR disLine);
    void __fastcall OutputMemAdr16(int argno, LPTSTR dst, DWORD arg, bool f1, bool f2, PDISINFO pDisInfo, LPTSTR disLine);
    void __fastcall OutputMemAdr32(int argno, LPTSTR dst, DWORD arg, bool f1, bool f2, PDISINFO pDisInfo, LPTSTR disLine);
};
//---------------------------------------------------------------------------
#endif
