...
//---------------------------------------------------------------------------
bool __fastcall TDfm::IsFormComponent(AnsiString CompName)
{
...
{
    int     n, pos, align;
    AnsiString  componentName, componentClass, eventName, procName;
    for (n = From; n < FormText->Count; n++)
    {
        AnsiString line = FormText->Strings[n].Trim();
        if (line == "") continue;
...
//---------------------------------------------------------------------------
bool __stdcall EnumResNameProcedure(int hModule, LPTSTR Type, LPTSTR Name, long Param)
{
	bool                res;
	TFilerFlags         flags;
	...
	AnsiString          className, vmtName;
	BYTE                signature[4];
	if (Type == RT_RCDATA || Type == RT_BITMAP)
	{
		LPWSTR name = TransformPWideChar(CP_ACP, Name, -1);
		resStream = new TResourceStream(hModule, name, (LPWSTR)Type);
		delete[] name;
		if (Type == RT_RCDATA)
		{
			...
			if (ResInfo->hFormPlugin)
			{
				...
				if (res)
				{
					...
					BYTE len = *(mp + 4);
					res = (len == strlen(Name) && SameText(Name, AnsiString((LPTSTR)mp + 5, len)));
				}
			}
			if (res)
			{
				...
				if (signature[0] == 'T' && signature[1] == 'P' && signature[2] == 'F' && (signature[3] >= '0' && signature[3] <= '7'))
				{
					if (signature[3] == '0')
					{
						...
						AnsiString line = formText->Strings[0];
						bool inherited = (line.Pos("inherited ") == 1);
						if (inherited)
...
//---------------------------------------------------------------------------
bool __fastcall TResourceInfo::EnumResources(AnsiString FileName)
{
	citadel = false;
	...
	if (hInst)
	{
		EnumResourceNames(hInst, RT_RCDATA, (ENUMRESNAMEPROCA)EnumResNameProcedure, (long)this);
		FreeLibrary(hInst);
		return true;
	}
    if (hFormPlugin)
...
{
    AnsiString parentName = GetParentName(Dfm->ClassName);
    int cnt = FormList->Count;
    for (int n = 0; n < cnt; n++)
    {
...
void __fastcall TResourceInfo::ReopenAllForms()
{
    ...
    for (int n = 0; n < cnt; n++)
    {
        ...
        if (dfm->Open == 2) //still opened
        {
            ...
            {
                AnsiString FormName = dfm->Form->Name;
                dfm->Form->Close();
                //this is a must have call!
                //it removes just closed form from the Screen->Forms[] list!
                //and we'll not have a new form with name [old_name]_1 ! 
...
void __fastcall TResourceInfo::InitAliases()
{
    ...
    for (int n = 0; n < cnt; n++)
    {
        AnsiString className = Aliases->Strings[n];
        if (!className.Pos("="))
        {
            AnsiString alias = "";
            try
...
//---------------------------------------------------------------------------
AnsiString __fastcall TResourceInfo::GetAlias(AnsiString ClassName)
{
...
//---------------------------------------------------------------------------
TDfm* __fastcall TResourceInfo::GetFormIdx(AnsiString FormName, int* idx)
{
...
//---------------------------------------------------------------------------
TDfm* __fastcall TResourceInfo::GetFormByClassName(AnsiString ClassName)
{
    for (int n = 0; n < FormList->Count; n++)
    {
    	TDfm* dfm = (TDfm*)FormList->Items[n];
...
//---------------------------------------------------------------------------
void __fastcall TResourceInfo::GetEventsList(AnsiString FormName, TList* Lst)
{
    ...
    if (dfm->Events->Count)
    {
        ...
        for (int k = 0; k < evCnt; k++)
        {
            ...
            AnsiString methodName = dfm->ClassName + "." + eInfo->ProcName;
            //Find the corresponding method's address
    ...
    for (int m = 0; m < compCnt; m++)
    {
        ...
        for (int k = 0; k < evCnt; k++)
        {
            ...
            if (SameText(eInfo->EventName, "Action"))
            {
                ...
                if (dotpos)
                {
                    AnsiString moduleName = eInfo->ProcName.SubString(1, dotpos - 1);
                    AnsiString actionName = eInfo->ProcName.SubString(dotpos + 1, eInfo->ProcName.Length() - dotpos);
...
void __fastcall IdrDfmForm::SetupControls()
{
    ...
    for (int n = 0; n < ComponentCount; n++)
    {
        ...
        const AnsiString curName = component->Name;
        SetupControlResetShortcut(component);
...
//---------------------------------------------------------------------------
void __fastcall IdrDfmForm::ShowMyPopupMenu(AnsiString FormName, AnsiString ControlName, bool show)
{
    int         n;
    TMenuItem   *mi;
    ...
    {
        //Inherited - Start filling out menu
        if (dfm->Flags & FF_INHERITED)
        {
        ...
        //First row of menu - Control name
        mi = new TMenuItem(evPopup);
        mi->Caption = dfm->Name + "." + dfm->ClassName;
        mi->Tag = classAdr;
        mi->OnClick = miPopupClick;
        evPopup->Items->Add(mi);
        //Second line - Separator menu
        mi = new TMenuItem(evPopup);
        mi->Caption = "-";
        evPopup->Items->Add(mi);
        PInfoRec recN = (IsValidImageAdr(classAdr)) ? Infos[Adr2Pos(classAdr)] : 0;
    ...
    else
    {
        ...
        if (dfm->Flags & FF_INHERITED)
        {
            ...
            //?конц?добавляем разделител?            mi = new TMenuItem(evPopup);
            mi->Caption = "-";
            evPopup->Items->Add(mi);
        }
        for (int m = 0; m < dfm->Components->Count; m++)
        {
            ...
            if (SameText(cInfo->Name, ControlName))
            {
                ...
                for (int k = 0; k < ev->Count; k++)
                {
                    ...
                    if (SameText(eInfo->EventName, "Action"))
                    {
                        //Name eInfo->ProcName maybe it's FormName.Name
                        int dotpos = eInfo->ProcName.Pos(".");
                        if (dotpos)
                        {
                            AnsiString moduleName = eInfo->ProcName.SubString(1, dotpos - 1);
                            AnsiString actionName = eInfo->ProcName.SubString(dotpos + 1, eInfo->ProcName.Length() - dotpos);
                            //Searching for Shapes moduleName
                            for (int j = 0; j < ResInfo->FormList->Count; j++)
                            {
                                ...
                                if (SameText(dfm1->Name, moduleName))
                                {
                                    classAdr = GetClassAdr(dfm1->ClassName);
                                    recN = (IsValidImageAdr(classAdr)) ? Infos[Adr2Pos(classAdr)] : 0;
                                    //Search component actionName
                                    for (int r = 0; r < dfm1->Components->Count; r++)
                                    {
                                        PComponentInfo cInfo1 = (PComponentInfo)dfm1->Components->Items[r];
                                        if (SameText(cInfo1->Name, actionName))
                                        {
                                            //Searching for events OnExecute
                                            TList* ev1 = cInfo1->Events;
                        ...
                        {
                            //Search component eInfo->ProcName
...
//---------------------------------------------------------------------------
void __fastcall IdrDfmForm::SetupControlHint(AnsiString FormName, TControl* Control, AnsiString InitHint)
{
    for (int n = 0; n < ResInfo->FormList->Count; n++)
    {
    	TDfm* dfm = (TDfm*)ResInfo->FormList->Items[n];
        //Find your own shape control
        if (SameText(dfm->Name, FormName))
        {
        	AnsiString hint = InitHint;
            //The form itself?
            ...
            else
            {
                ...
                {
                    PComponentInfo cInfo = (PComponentInfo)dfm->Components->Items[m];
                    //Find the component that belongs to control
...
void __fastcall IdrDfmForm::SetupMenuItem(TMenuItem* mi, AnsiString searchName)
{
    ...
    for (int n = 0; n < ResInfo->FormList->Count; n++)
    {
        ...
        if (SameText(dfm->Name, searchName))
        {
            ...
            {
                PComponentInfo cInfo = (PComponentInfo)dfm->Components->Items[m];
                //Find components belonging to mi
                if (SameText(cInfo->Name, mi->Name))
                {
                    ...
                    for (int k = 0; k < ev->Count; k++)
                    {
                        PEventInfo eInfo = (PEventInfo)ev->Items[k];
                        //OnClick
                        AnsiString methodName = ""; PMethodRec recM;
                        ...
                        if (SameText(eInfo->EventName, "Action"))
                        {
                            //Find components with names eInfo->ProcName
                            for (int r = 0; r < dfm->Components->Count; r++)
                            {
                                PComponentInfo cInfo1 = (PComponentInfo)dfm->Components->Items[r];
                                if (SameText(cInfo1->Name, eInfo->ProcName))
...
//---------------------------------------------------------------------------
void __fastcall IdrDfmForm::DoFindMethod(AnsiString& methodName)
{
	AnsiString name = getFormName();
	if (FOnFindMethod) FOnFindMethod(this, name, methodName);
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
AnsiString __fastcall IdrDfmForm::getFormName()
{
    return AnsiString("T") + originalClassName;
}
//---------------------------------------------------------------------------
...
void __fastcall IdrDfmLoader::FindComponentClass(TReader* Reader, String ClassName, TMetaClass* &ComponentClass)
{
    ...
    catch (Exception& e)
    {
        ...
        AnsiString alias = ResInfo->GetAlias(ClassName);
        for (int n = 0;;n++)
        {
            if (!RegClasses[n].RegClass) break;
...
//---------------------------------------------------------------------------
bool __fastcall IdrDfmLoader::HasGlyph(AnsiString ClassName)
{
    for (int n = 0; n < ResInfo->FormList->Count; n++)
    {
    	TDfm* dfm = (TDfm*)ResInfo->FormList->Items[n];
...
//---------------------------------------------------------------------------
void IdrDfmDefaultControl::SetClassName(const AnsiString& name, const AnsiString& mappedName)
{
    ...
    //so I expect to have a AnsiString "TOpenDialog in mappedName, not "Default" !
    
    mappedClassName = name;
    AnsiString usedClassName = (mappedClassName.IsEmpty()) ? originalClassName : mappedClassName;
...
//---------------------------------------------------------------------------
bool IdrDfmDefaultControl::HasIconForClass(const AnsiString& name)
{
    return true;
...
//---------------------------------------------------------------------------
void IdrDfmDefaultControl::CreateImageIconForClass(const AnsiString& imgFile)
{
