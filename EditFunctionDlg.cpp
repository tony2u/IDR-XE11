...
void __fastcall TFEditFunctionDlg_11011981::bApplyTypeClick(TObject *Sender)
{
	...
	int newEndAdr = 0;
    if (lEndAdr->Text == "" || !TryStrToInt(AnsiString("$") + lEndAdr->Text, newEndAdr))
    {
        ShowMessage("End address is not valid");
        return;
    }
    newEndAdr = StrToInt(AnsiString("$") + lEndAdr->Text);
    ...
    if (lStackSize->Text == "" || !TryStrToInt(AnsiString("$") + lStackSize->Text, StackSize))
    {
        ShowMessage("StackSize not valid");
        return;
    }
    StackSize = StrToInt(AnsiString("$") + lStackSize->Text);
    ...
    AnsiString retType = recN->procInfo->AddArgsFromDeclaration(p, from, rgCallKind->ItemIndex);
...
{
    int     recofs, size;
    AnsiString  fname, ftype, name, type;
            ...
            if (KnowledgeBase.GetTypeInfo(idx, INFO_FIELDS, &tInfo))
            {
                if (tInfo.FieldsNum)
                {
                    ...
                    for (int k = 0; k < tInfo.FieldsNum; k++)
                    {
                        ...
                        //Name
                        int len = *((WORD*)p); p += 2;
                        name = AnsiString((LPTSTR)p, len); p += len + 1;
                        //Type
                        len = *((WORD*)p); p += 2;
                        type = TrimTypeName(AnsiString((LPTSTR)p, len)); p += len + 1;
