...
void __fastcall TFMain_11011981::ShowUnits(bool showUnk)
{
    ...
    if (oldItemIdx != -1)
    {
		...
		LPTSTR text = TransformPChar(CP_ACP, item.c_str(), -1);
		sscanf(text + 1, "%lX", &selAdr);
		delete[] text;
    }
...
void __fastcall TFMain_11011981::miRenameUnitClick(TObject *Sender)
{
    ...
    String item = lbUnits->Items->Strings[lbUnits->ItemIndex];
	DWORD adr;
	LPTSTR _text = TransformPChar(CP_ACP, item.c_str(), -1);
	sscanf(_text + 1, "%lX", &adr);
	delete[] _text;
    PUnitRec recU = GetUnit(adr);
...
void __fastcall TFMain_11011981::lbUnitItemsDblClick(TObject *Sender)
{
    ...
    //Xrefs?
    if (item[11] == '<' || item[11] == '?')
		sscanf(text + 1, "%lX%s%s", &adr, tkName, typeName);
    else
		sscanf(text + 1, "%lX%d%s%s", &adr, &refCnt, tkName, typeName);
	delete[] text;
...
void __fastcall TFMain_11011981::miEditFunctionIClick(TObject *Sender)
{
	...
	String item = lbUnitItems->Items->Strings[lbUnitItems->ItemIndex];
	LPTSTR text_ = TransformPChar(CP_ACP, item.c_str(), -1);
    //Xrefs?
    if (item[11] == '<' || item[11] == '?')
		sscanf(text_ + 1, "%lX%s", &adr, tkName);
    else
		sscanf(text_ + 1, "%lX%d%s", &adr, &refCnt, tkName);
    delete[] text_;
    String name = String(tkName);
