...
//---------------------------------------------------------------------------
extern  int         dummy;
extern  AnsiString  IDRVersion;
extern  AnsiString  SelectedAsmItem;
...
void __fastcall FillArgInfo(int k, BYTE callkind, PARGINFO argInfo, BYTE** p, int* s)
{
    ...
    argInfo->Name = AnsiString((LPTSTR)pp, wlen); pp += wlen + 1;
    wlen = *((WORD*)pp); pp += 2;
    argInfo->TypeDef = TrimTypeName(AnsiString((LPTSTR)pp, wlen)); pp += wlen + 1;
    *p = pp; *s = ss;
}
//---------------------------------------------------------------------------
AnsiString __fastcall TrimTypeName(const AnsiString& TypeName)
{
    ...
    if (pos == 0 || pos == 1)
        return TypeName;
    //Or the name of the type range
    else if (TypeName[pos + 1] == '.')
        return TypeName;
    else
    {
        TCHAR c, *p = TypeName.c_str();
...
int __fastcall StrGetRecordSize(AnsiString str)
{
    ...
    if (_bpos && _epos)
    {
        AnsiString _sz = str.SubString(_bpos + 5, _epos - _bpos - 5);
        _size = StrToInt("$" + _sz);
    }
    return _size;
}
...
int __fastcall StrGetRecordFieldOffset(AnsiString str)
{
    ...
    if (_bpos && _epos)
    {
        AnsiString _ofs = str.SubString(_bpos + 2, _epos - _bpos - 2);
...
int __fastcall GetRecordField(AnsiString ARecType, int AOfs, AnsiString& name, AnsiString& type)
{
    ...
    PTypeRec    _recT;
    MTypeInfo   _tInfo;
    AnsiString      _str, _prevstr, _name, _typeName, _ofs, _sz, _result = "";
                            ...
                            for (n = 0, k = 0; n < _tInfo.FieldsNum; n++)
                            {
                                ...
                                if (_case == _cases[m].caseno)
                                {
                                    ...
                                    if (AOfs >= Ofs1 && AOfs < Ofs2)
                                    {
                                        ...
                                        p += 4;//case
                                        _len = *((WORD*)p); p += 2;
                                        name = AnsiString(p, _len); p += _len + 1;
                                        _len = *((WORD*)p); p += 2;
                                        type = AnsiString(p, _len); p += _len + 1;
                                        return Ofs1;
                                    }
                                    k++;
                                }
    ...
    if (_recT && _recT->kind == ikRecord)
    {
        ...
        _pos++;//TypeKind
        _len = Code[_pos]; _pos++;
        _name = AnsiString((LPTSTR)(Code + _pos), _len); _pos += _len;//Name
        _pos += 4;//Size
        ...
        if (DelphiVersion >= 2010)
        {
            ...
            for (n = 0; n < _elNum; n++)
            {
                ...
                _pos++;//Flags
                _len = Code[_pos]; _pos++;
                _name = AnsiString((LPTSTR)(Code + _pos), _len); _pos += _len;
                //AttrData
                _dw = *((WORD*)(Code + _pos));
                _pos += _dw;//ATR!!
                if (n == _elNum - 1)
...
int __fastcall GetField(AnsiString TypeName, int Offset, AnsiString& name, AnsiString& type)
{
    int         size, kind, ofs;
    PFIELDINFO	fInfo;
    AnsiString      _fname, _ftype, _type = TypeName;
    _fname = ""; _ftype = "";
    while (Offset >= 0)
    {
...
//---------------------------------------------------------------------------
AnsiString __fastcall GetRecordFields(int AOfs, AnsiString ARecType)
{
    AnsiString      _name, _typeName;
...
{
    assert(Idx >= 0 && Idx < 32);
    if (Idx >= 31) return "st(" + AnsiString(Idx - 31) + ")";
    if (Idx == 30) return "st";
    if (Idx >= 24) return AnsiString(SegRegTab[Idx - 24]);
    if (Idx >= 16) return AnsiString(Reg32Tab[Idx - 16]);
    if (Idx >= 8) return AnsiString(Reg16Tab[Idx - 8]);
    return AnsiString(Reg8Tab[Idx]);
}
//---------------------------------------------------------------------------
...
AnsiString __fastcall GetClsName(DWORD adr)
{
    ...
    return AnsiString((LPTSTR)&Code[pos], len);
}
//---------------------------------------------------------------------------
...
{
	bool        s = true;//true - print AnsiString, false - print #XX
    BYTE        c, *p = str;
    AnsiString      res = "";
    for (int k = 0; k < len; k++)
    {
...
AnsiString __fastcall TransformUString(WORD codePage, wchar_t* data, int len)
{
    ...
    if (!nChars) return "";
    LPTSTR tmpBuf = new TCHAR[nChars + 1];
    WideCharToMultiByte(codePage, 0, data, -1, tmpBuf, nChars, 0, 0);
    tmpBuf[nChars] = 0;
    AnsiString res = QuotedStr(tmpBuf);
    delete[] tmpBuf;
    return res;
}
//---------------------------------------------------------------------------
...
{
	if (!IsValidCodePage(codePage)) codePage = CP_ACP;
	int nChars = MultiByteToWideChar(codePage, 0, data, -1, 0, 0);
	LPWSTR tmpBuf=new wchar_t[nChars];
	wmemset(tmpBuf, 0, nChars);
    MultiByteToWideChar(codePage, 0, data, -1, tmpBuf, nChars);
	return tmpBuf;
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
AnsiString __fastcall GetTypeName(DWORD adr)
{
	if (!IsValidImageAdr(adr)) return "?";
    ...
    BYTE len = *(Code + pos); pos++;
    AnsiString Result = AnsiString((LPTSTR)(Code + pos), len);
    if (Result[1] == '.')
    {
        ...
        if (recU)
        {
            ...
            Result = prefix + AnsiString((int)recU->iniOrder) + "_" + Result.SubString(2, len);
        }
    }
    return Result;
}
...
AnsiString TypeKinds[TypeKindsNum] =
{
    "Unknown",
    "Integer",
    "Char",
...
BYTE __fastcall GetTypeKind(AnsiString AName, int* size)
{
    ...
    WORD*       uses;
    MTypeInfo   tInfo;
    AnsiString      name, typeName, str, sz;
    *size = 4;
    if (AName != "")
    {
        if (AName.Pos("array"))
...
//---------------------------------------------------------------------------
//return AnsiString representation of immediate value with comment
AnsiString __fastcall GetImmString(int Val)
{
    AnsiString _res;
    AnsiString _res0 = AnsiString((int)Val);
    if (Val > -16 && Val < 16) return _res0;
    _res = AnsiString("$") + Val2Str0(Val);
...
{
    int     _size;
    AnsiString  _str, _default = GetImmString(Val);
    ...
    if (_kind == ikChar) return Format("'%s'", ARRAYOFCONST(((TCHAR)Val)));
    return _default;
}
//---------------------------------------------------------------------------
...
AnsiString __fastcall GetEnumerationString(AnsiString TypeName, Variant Val)
{
    ...
    if (IsValidImageAdr(adr))
    {
        ...
        pos++;
        len = Code[pos]; pos++;
        clsName = AnsiString((LPTSTR)(Code + pos), len); pos += len;
        //ordType
        pos++;
...
//---------------------------------------------------------------------------
/*
AnsiString __fastcall GetArrayElement(AnsiString arrType, int offset)
{
    AnsiString  _result = "";
...
bool __fastcall IsBplByExport(const LPTSTR bpl)
{
	...
	if (hLib)
    {
			...
			pHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].Size)
		{
			...
			szDll = (LPTSTR)(imageBase + pExport->Name);
			...
			for (int i = 0; i < pExport->NumberOfFunctions; i++)
			{
				...
				if (index != -1)
				{
                    ...
                    AnsiString curFunc = AnsiString((LPTSTR)imageBase + pName);
...
AnsiString __fastcall UnmangleName(AnsiString Name)
{
    int     pos;
    //skip first '@'
    AnsiString  Result = Name.SubString(2, Name.Length());
...
bool __fastcall IsDefaultName(AnsiString AName)
{
    ...
    {
        if (SameText(AName, AnsiString(Reg32Tab[Idx]))) return true;
    }
