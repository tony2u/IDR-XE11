...
void __fastcall TFTypeInfo_11011981::ShowKbInfo(MTypeInfo* tInfo)
{
    ...
    if (tInfo->FieldsNum)
    {
        ...
        for (int n = 0; n < tInfo->FieldsNum; n++)
        {
            ...
            if (fInfo.Case != -1) line += " case " + AnsiString(fInfo.Case);
    ...
    if (tInfo->PropsNum)
    {
        ...
        for (int n = 0; n < tInfo->PropsNum; n++)
        {
            ...
            Len = *((WORD*)p); p += 2;
            pInfo.Name = AnsiString((LPTSTR)p, Len); p += Len + 1;
            Len = *((WORD*)p); p += 2;
            pInfo.TypeDef = AnsiString((LPTSTR)p, Len); p += Len + 1;
            Len = *((WORD*)p); p += 2;
            pInfo.ReadName = AnsiString((LPTSTR)p, Len); p += Len + 1;
            Len = *((WORD*)p); p += 2;
            pInfo.WriteName = AnsiString((LPTSTR)p, Len); p += Len + 1;
            Len = *((WORD*)p); p += 2;
            pInfo.StoredName = AnsiString((LPTSTR)p, Len); p += Len + 1;
    ...
    if (tInfo->MethodsNum)
    {
        ...
        for (int n = 0; n < tInfo->MethodsNum; n++)
        {
            ...
            Len = *((WORD*)p); p += 2;
            mInfo.Prototype = AnsiString((LPTSTR)p, Len); p += Len + 1;
...
//---------------------------------------------------------------------------
AnsiString __fastcall GetVarTypeString(int val)
{
    AnsiString  res = "";
    switch (val)
    {
...
//---------------------------------------------------------------------------
AnsiString __fastcall TFTypeInfo_11011981::GetRTTI(DWORD adr)
{
    bool        found;
    ...
    BYTE        GUID[16];
    AnsiString		typname, name, FldFileName;
    FILE        *fldf;
    PInfoRec    recN;
    ...
    RTTIName = AnsiString((LPTSTR)(Code + pos), len); pos += len;
    Caption = RTTIName;
    AnsiString result = "", proto;
    ...
    case ikEnumeration:
        ...
        for (i = minValueB; i <= maxValueB; i++)
        {
            ...
            {
                name = AnsiString((LPTSTR)(Code + pos), len);
        ...
        {
            pos++;
            Caption = AnsiString((LPTSTR)(Code + pos), len).Trim() + "." + Caption;
        }
        break;
    case ikFloat:
    ...
    case ikClass:
        ...
        for (i = 0; i < propCount; i++)
        {
            ...
            posn++; //property type
            len = Code[posn]; posn++;
            typname = AnsiString((LPTSTR)(Code + posn), len);
                ...
                if (procSig)
                {
                    ...
                    if (flags != 0xFF)
                    {
                        ...
                        for (i = 0; i < paramCount; i++)
                        {
                            ...
                            len = Code[posn]; posn++;
                            name = AnsiString((LPTSTR)(Code + posn), len); posn += len;
    ...
    case ikArray:
        ...
        result = "array [1.." + AnsiString(elNum);
        if (DelphiVersion >= 2010)
        {
            result = "array [";
            //DimCount
            ...
            for (i = 0; i < dimCount; i++)
            {
                ...
                else
                {
                    ...
                    {
                        if (dimCount == 1)
                            result += "1.." + AnsiString(elNum);
                        else
                            result += "1..?";
                    }
                    else
                        result += "1.." + AnsiString(typeAdr);
                }
    ...
    case ikInterface:
        ...
        //Methods
        propCount = *((WORD*)(Code + pos)); pos += 2;
        result += "Methods Count = " + AnsiString(propCount);
        if (DelphiVersion >= 6)
        {
            //RttiCount
            ...
            if (dw != 0xFFFF)
            {
                if (DelphiVersion >= 2010)
                {
                    for (i = 0; i < propCount; i++)
                    {
                        //Name
                        len = Code[pos]; pos++;
                        result += "\n" + AnsiString((LPTSTR)(Code + pos), len); pos += len;
                        //Kind
                        methodKind = Code[pos]; pos++;
                        //CallConv
                        pos++;
                        ...
                        for (m = 0; m < paramCount; m++)
                        {
                            ...
                            //ParamName
                            len = Code[pos]; pos++;
                            result += AnsiString((LPTSTR)(Code + pos), len); pos += len;
                            //TypeName
                            len = Code[pos]; pos++;
                            result += ":" + AnsiString((LPTSTR)(Code + pos), len); pos += len;
                            //ParamType
        ...
        if (procSig)
        {
            ...
            if (flags != 0xFF)
            {
                ...
                for (i = 0; i < paramCount; i++)
                {
                    ...
                    //Name
                    len = Code[pos]; pos++;
                    result += AnsiString((LPTSTR)(Code + pos), len) + ":";
                    pos += len;
                    result += GetTypeName(typeAdr);
